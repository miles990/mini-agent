...
export function registerPrimitive(name: string, fn: PrimitiveFn): void { primitives.set(name, fn); }

// ── Built-in Primitives ──
primitives.set('file-exists', async (args, cwd) => {
  const ok = fs.existsSync(path.resolve(cwd, args[0]));
...
primitives.set('process-running', async (args, cwd) => {
  try {
