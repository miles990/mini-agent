...
- 需要深度工作 → 用 delegate 委派背景，不在此 lane 長時間推理
  ...
  private async cycle(): Promise<string | null> {
    ...
    try {
      ...
      slog(`LOOP`, `[ctx-size] cycle=${this.cycleCount} mode=${contextMode} chars=${context.length} tok≈${Math.round(context.length/4)}`);
      ...
      try {
        ...
        const failedProducts = workClosure.productVerifiers
          ...
          .join(',');
        if (failedProducts) {
          slog('WORK-CLOSURE', `product verifier failed: ${failedProducts}`);
        }
