...
export function buildTaskQueueSection(memoryDir: string): string {
  ...
  const lines = items.map(item => {
    ...
    const teamLoop = payload.team_loop ? ' | report=progress+completion | kg=link' : '';
    return `- [${item.status}] (${item.type}) ${item.summary ?? item.id} | id=${item.id}${role}${teamLoop} | ${verifyStr}${stale}`;
  });
...
export function buildTaskQueueSectionCompact(memoryDir: string): string {
  ...
  const lines = items.map(item => {
    ...
    const role = typeof payload.role === 'string' ? ` role=${payload.role}` : '';
    const teamLoop = payload.team_loop ? ' report=progress+completion kg=link' : '';
    return `- [${item.status}] ${summary}${staleMarker} | id=${item.id}${role}${teamLoop} | ${verifyStr}`;
  });
...
const META_FEEDBACK_RE = /^(好的?|OK|收到|了解|不錯|沒問題|對|嗯|繼續|加油|怎麼做最好|用最好的方式|你自[主己]|做得[好不])/i;
const GENERAL_DIRECTIVE_RE = /^.{0,40}(你自[主己]來?決定|你(自己)?判斷|交給你|隨你|你(來)?決定|你看著辦)/;
const INTERNAL_ROUTING_RE = /^\[表達意圖\]|^\[OODA|phase:(observe|orient|decide|act)|routing:internal/i;
const BARE_EXECUTION_NUDGE_RE = /^(直接開始做|做|幫我做|開始|go|do it|執行)$/i;

function isNonTask(message: string): boolean {
  const trimmed = message.trim();
  if (INTERNAL_ROUTING_RE.test(trimmed)) return true;
  if (BARE_EXECUTION_NUDGE_RE.test(trimmed)) return true;
  if (META_FEEDBACK_RE.test(trimmed) && trimmed.length < 50 && !URL_RE.test(trimmed) && !CONCRETE_VERBS.test(trimmed)) return true;
  if (GENERAL_DIRECTIVE_RE.test(trimmed) && trimmed.length < 60 && !URL_RE.test(trimmed)) return true;
  return false;
}
...
function extractUrlContext(url: string): string {
  ...
  if (domain.includes('github.com') && parts.length >= 3) return `${parts[1]}/${parts[2]}`;
  if (domain.includes('x.com') || domain.includes('twitter.com')) {
    return parts[1] ? `@${parts[1]}` : domain;
  }
  if (parts.length >= 2) return `${domain}/${parts[1].slice(0, 30)}`;
  return domain;
}

function extractTopicHint(message: string): string {
  const cleaned = message.replace(URL_RE, '').replace(/\n/g, ' ').trim();
  if (cleaned.length > 5) return cleaned.slice(0, 40).trim();
...
function ruleDecompose(message: string, intent: IntentType): Array<{ title: string; type: IntentType; role?: string; acceptance?: string }> | null {
  if (intent === 'research' && URL_RE.test(message)) {
    ...
    return [
      { title: `研究 ${ctx} — 閱讀分析`, type: 'research' },
      { title: `撰寫分析: ${topic}`, type: 'execute' },
...
export async function enqueueRoomDirective(
  ...
  // Dedup gate: skip if a pending/in_progress task with same summary exists
  ...
  if (shouldDecompose(message) || shouldPmDecompose(message) || intent === 'research') {
    ...
    if (subtasks && subtasks.length > 1) {
      ...
      for (const sub of subtasks) {
        if (isDup(sub.title)) {
          slog('NEXT', `Dedup skip: "${sub.title.slice(0, 40)}"`);
          continue;
        }
  ...
  if (isDup(title)) {
    slog('NEXT', `Dedup skip [${from}] "${title.slice(0, 40)}"`);
    return;
  }
...
export async function pruneNonActionableRoomTasks(
  ...
  for (const task of tasks) {
    ...
    const text = String(task.summary ?? '');
    if (!isNonTask(text)) continue;
    await updateMemoryIndexEntry(memoryDir, task.id, {
      status: 'abandoned',
      payload: {
        ...payload,
        pruned_reason: 'non-actionable-room-directive',
        pruned_at: new Date().toISOString(),
      },
    });
    pruned++;
  }
  if (pruned > 0) slog('NEXT', `Pruned ${pruned} non-actionable room task(s)`);
  return { pruned };
}
