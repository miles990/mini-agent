...
const EXPLORE_BUDGET = 0.2;       // 20% of fetches for exploration
...
export function getNutrientStats(): {
  ...
  exploring: number;
  pruned: number;
  topDomains: Array<{ domain: string; score: number; yield: string }>;
  prunedDomains: string[];
} {
  try {
    const scores = computeScores();
    let reinforced = 0, exploring = 0, pruned = 0;
    const topList: Array<{ domain: string; score: number; yield: string }> = [];
    ...
    for (const [, s] of scores) {
      if (s.tier === 'reinforce') {
        ...
        topList.push({
          domain: s.domain,
          score: s.score,
          yield: `${(s.nutrientYield * 100).toFixed(0)}%`,
        });
      } else if (s.tier === 'prune') {
        pruned++;
        prunedList.push(s.domain);
      } else {
        exploring++;
      }
    }
    ...
    return {
      totalDomains: scores.size,
      reinforced,
      exploring,
      pruned,
      topDomains: topList.slice(0, 5),
      prunedDomains: prunedList.slice(0, 5),
    };
  } catch {
    return { totalDomains: 0, reinforced: 0, exploring: 0, pruned: 0, topDomains: [], prunedDomains: [] };
  }
}
