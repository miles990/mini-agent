...
| InboxProcessor | `src/inbox-processor.ts` |
| CycleTasks | `src/cycle-tasks.ts` |
| CycleState | `src/cycle-state.ts` |
| PromptBuilder | `src/prompt-builder.ts` |
| MushiClient | `src/mushi-client.ts` |
| IPC Bus | `src/ipc-bus.ts` |
| TaskRouter | `src/task-router.ts` |
| Scaling | `src/scaling.ts` |
| MeshHandler | `src/mesh-handler.ts` |
| Perspective | `src/perspective.ts` |
| Consensus | `src/consensus.ts` |
| MemoryCache | `src/memory-cache.ts` |
| Forge Lite | `scripts/forge-lite.sh` |
| Landlock Sandbox | `scripts/landlock-sandbox.py` |
...
因應 AI 環境快速變化的戰略方向。不是 premature abstraction — 是讓 Kuro 能快速吸收養分的基礎設施（multi-lane delegation、parallel tentacles 都建立在此之上）。

**架構**：8 個核心模組（~4K 行），整合到 loop.ts 三個點：

| 模組 | 功能 | 整合點 |
|------|------|--------|
| `ipc-bus.ts` | 跨進程 file-based event bus | 啟動時 initIPCBus() |
| `memory-cache.ts` | CQRS read cache + fswatch invalidation | memory.ts, api.ts |
| `task-router.ts` | 任務路由 + mushi route 整合 | loop.ts:~1310（每 cycle 開始） |
| `scaling.ts` | 動態 scaling controller | loop.ts:~1670（每 cycle 結束） |
| `mesh-handler.ts` | 路由執行（forward/spawn/queue） | task-router 呼叫 |
| `perspective.ts` | Mesh output + 結果回流 + buildContextForPerspective | loop.ts:~1374（context 注入） |
| `consensus.ts` | Decision journal + exclusive claims | api.ts, index.ts |
| `filelock.ts` | 跨進程 file lock（dual-layer） | 共享資源存取 |

**設計原則**：
- **Lazy activation** — 單 instance 時最小化 overhead，偵測到 peer 才啟動完整邏輯
- **Option value > Carrying cost** — AI agent 生態快速演化，有基礎設施才能快速適應
- **不只是 clustering** — 支援 specialist agents、跨機器分佈、外部 agent 互通、動態資源調配
- Feature flag: `cognitive-mesh`

**Specialist Context 分流**：`buildContextForPerspective()` 為 specialist instance 提供精簡 context（~5-10K chars），相比 primary 的完整 context（~50K chars）節省 >60% tokens。

**API**：`POST /api/mesh/task` — 接收 specialist 轉發的任務

## Forge — Worktree Isolation for Delegations

`scripts/forge-lite.sh` 提供 crash-proof 的 git worktree 隔離，讓 delegation subprocess 在獨立環境中工作。

**三層安全模型**：
1. **Kernel sandbox** — macOS: `sandbox-exec`（`(deny default)` + whitelist）/ Linux: Landlock LSM（`scripts/landlock-sandbox.py`）
2. **Breach detection** — delegation 完成後 `git diff` 比對 main dir，偵測 sandbox 外洩的檔案變更
3. **Prompt hints** — subprocess 被告知只能修改 worktree 內的檔案

**Slot 管理**：3 個持久 slot（`FORGE_SLOTS`），cached `node_modules`。Slot 狀態透過 `.forge-in-use` marker 追蹤（branch + caller PID + timestamp）。

**Agent-friendly 輸出**：
- `--json` flag（`create`、`status`）回傳結構化 JSON
- stdout = 可解析結果，stderr = 日誌
- `status`/`list`/`ls` 別名

**Decay 策略**：失敗的 delegation → subprocess PID 死亡 → `is_slot_abandoned()` 偵測 → 下次 `create` 自動回收 slot。`delegation.ts` 不自行管理 decay timer，全部由 forge-lite.sh 的內建機制處理。

**環境變數**：

| 變數 | 說明 | 預設 |
|------|------|------|
| `FORGE_SLOTS` | 持久 worktree slot 數量 | 3 |
| `FORGE_SLOT_TTL_MINUTES` | Slot abandoned 判定時間（PID check 的 fallback） | 60 |
| `FORGE_STALE_HOURS` | 自動清理超過此時間的 worktree | 24 |

**指令**：`create`、`verify`、`merge`、`yolo`（verify+merge）、`cleanup`、`status`、`recover`、`help`
...
**反模式（轉變前）**：
- 收到指令 → 重複回答已經回答過的問題（Cognitive Mesh 完成度被問了多次，每次重新盤點）
- 做表面任務（typecheck 通過 = 完成，不看實際運行證據）
- 被「最明顯的事」吸引（Show HN 數據、回答 inbox），而非「最重要的事」（架構審視）
- 想完再做（分析 → 規劃 → 等下個 cycle 才行動）

**正確模式（轉變後）**：
- 找槓桿點：loop.ts 3,413 行是系統最大的單體 → 模組化是複利
- 邊做邊想：五刀提取，每刀 typecheck + commit，過程中發現下一刀目標
- 退後一步看全貌：產出架構審視 proposal，從「做什麼」轉向「為什麼做」
- 自我修正：承認之前「Phase 2-4 有骨架但沒接通」的判斷是錯的

**轉變的關鍵推力**：Alex 給的是原則和方向（「找複利」「黏菌模型」「大處著眼」），不是具體指令。行動是 Kuro 自己的。方向感來自外部推力，執行力來自內部判斷。

### 戰略思維

- **提案是戰略儲備，不是待辦清單** — 60+ 份提案 = 黏菌的 60 條觸手。有些會找到養分，有些不會。過時的要修剪，有潛力的要強化
- **option value > carrying cost** — 在快速變化的環境中，有基礎設施比沒有好。Cognitive Mesh 不是 premature abstraction，是因應未來的戰略方向
- **精煉的 ROI 遠高於擴展** — 讓 30K 行變成每一行都有理由存在的 25K 行，比加到 35K 行更有價值
- **系統性方法要自己長出來** — 沒有人能給你一個 pipeline 讓你變好。從實踐中觀察自己的模式，找到有效的，強化它
