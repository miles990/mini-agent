...
- **每次失敗都在限縮問題** — 失敗不是浪費，是排除。「curl 回傳空」排除了靜態頁面的可能 → JS 渲染 → Pinchtab。「AUTH_REQUIRED」排除了公開存取 → 需要 session。每次嘗試後，問題的可能原因變少了，下一步應該更精準
- **記錄每一步** — 所有操作的結果、耗時、錯誤都記到 log（`cdp.jsonl`）。出問題時這些記錄就是線索。不記 log 的失敗等於白失敗
