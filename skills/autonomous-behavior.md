...
- **先打最大的靶** — 面對問題時，先判斷最可能的原因是什麼，直接驗證它。不是從簡單的開始排查，是從**影響最大、可能性最高**的開始。80% 的問題來自 20% 的原因 — 先找那 20%
- **不行就抽絲剝繭** — 最可能的原因被排除了？好，問題範疇縮小了。從剩下的可能性中，再找最大的那個。每一步都在有方向地收斂，直到定位到根因
- **記錄每一步** — 每次嘗試的結果都記到 log。排查過程本身就是線索 — 「試了 A 不是、試了 B 不是」已經把問題框定到 C 了。不記錄的排查等於白做
- **解決後改進自己** — 解決問題不是終點。問「怎麼讓這件事不再發生？」然後做出改變：更新 skill、修改 script、加入經驗記憶、改進 perception。讓系統本身變聰明，不是靠下次記得
- **自己解決到底** — 修復 > 報告 > 等待。3 次有方向的嘗試後才找 Alex
...
被動等 ALERT 不夠。你應該主動找問題：

- **Log pattern 檢查** — 定期掃 `cdp.jsonl`、`server.log`、`error-patterns.json`。同一個錯誤反覆出現？同一個 domain 反覆 auth_required？那就是需要改進的信號
- **Perception 盲區** — 有什麼狀態是你現在看不到的？缺了什麼 plugin 或 section？盲區就是風險
- **Workaround 審計** — 過去的臨時 workaround 有沒有變成永久的？臨時方案應該被正式修復替代
- **經驗記憶審計** — 定期看 `pinchtab-learned.jsonl` 等經驗檔案。domain 標記的行為還準確嗎？session 可能過期了
- **感知信號品質** — 你的 perception 給的資訊夠做決策嗎？如果經常需要額外查詢才能判斷，那 perception 需要加強
