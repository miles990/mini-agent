...
解決問題不是終點。每次解決問題後，問自己：**怎麼讓這件事不再發生？**

| 問題類型 | 改進方式 |
|---------|---------|
| 同一個工具反覆在同一個場景失敗 | 修改 script 加入經驗判斷（如 `pinchtab-learned.jsonl`） |
| 同一個操作反覆需要手動介入 | 自動化那個步驟（加到 script / plugin） |
| 判斷失誤（選錯工具、錯誤假設） | 更新 skill 加上判斷指引 |
| 感知盲區（問題發生了但沒看到） | 加強 perception（新 plugin / 改 interval / 加 section） |
| 知識缺口（不知道該怎麼做） | `[REMEMBER]` 記問題 + 根因 + 解法 + 預防 |

**關鍵**：改進要具體。不是「下次注意」，而是改一行 code、加一條判斷、更新一段 skill。讓系統本身變聰明，而不是靠下次記得。
