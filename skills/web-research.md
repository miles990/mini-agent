...
處理步驟：
1. `bash scripts/pinchtab-fetch.sh fetch "URL"` — 自動 headless → visible 切換
2. 還是抓不到 → `node scripts/cdp-fetch.mjs fetch "URL"` — 直連 Chrome :9222（有 FB session）
3. 還是抓不到 → 直接問 Alex：「那篇貼文我看不到，在講什麼？」
4. ❌ 不要去訪問 `{page}` 對應的網站來推測內容
