...
import { callClaude, preemptLoopCycle, isLoopBusy, abortForeground, acquireForegroundSlot, releaseForegroundSlot, getLaneStatus } from './agent.js';
...
import { updateTemporalState, flushTemporalState, startThread, progressThread, completeThread, pauseThread, } from './temporal.js';
import { hasP0Tasks, getPendingTaskPreviews, getP0TaskPreviews, markTaskDoneByDescription } from './memory-index.js';
import { readPendingInbox, detectModeFromInbox, hasRecentUnrepliedTelegram, getUnprocessedHighPriority, queueInboxMark, flushInboxMarks } from './inbox.js';
...
import { runHousekeeping, autoPushIfAhead, trackTaskProgress, markTaskProgressDone } from './housekeeping.js';
...
import { routeTask, mushiRoute, getClusterState } from './task-router.js';
...
function getPendingForegroundDelegations() {
    try {
        const fpath = getForegroundDelegationsPath();
...
function clearForegroundDelegation(taskId) {
    try {
        const fpath = getForegroundDelegationsPath();
        if (!fs.existsSync(fpath))
            return;
...
class DelegationBatchBuffer {
    ...
    windowMs;
    onFlush;
    constructor(onFlush, windowMs = 10_000) {
        this.onFlush = onFlush;
        this.windowMs = windowMs;
    }
...
export class AgentLoop {
    static ACTION_SIMILARITY_THRESHOLD = 0.8;
    config;
    running = false;
    paused = false;
    ...
    nextCycleAt = null;
    cycling = false;
    hasPendingDelegationResults = false;
    // ── Continuation State ──
    consecutiveNowCount = 0;
    static MAX_CONSECUTIVE_NOW = 10;
    // ── Autonomous Mode State ──
    autonomousCooldown = 0;
    lastAutonomousActions = [];
    currentMode = 'idle';
    lastActionForSimilarity = null;
    metricsDate = new Date().toISOString().slice(0, 10);
    dailyTaskCycles = 0;
    dailyAutonomousCycles = 0;
    dailyRememberCount = 0;
    dailySimilaritySamples = 0;
    dailySimilarActions = 0;
    // ── Reflect nudge: track consecutive learn cycles ──
    consecutiveLearnCycles = 0;
    // ── Behavior config resilience ──
    lastValidConfig = null;
    // ── Cross-cycle state (only last cycle, no accumulation) ──
    previousCycleInfo = null;
    workJournalContext = null;
    // ── Interrupted cycle resume (Phase 1b + 1c) ──
    interruptedCycleInfo = null;
    // ── Foreground Reply (parallel response during cycling — concurrent batch+pool) ──
    foregroundReplyRecords = [];
    batchBuffer;
    delegationBatchBuffer;
    // ── Per-perception change detection (Phase 4) ──
    lastPerceptionVersion = -1;
    ...
    lastCompletedCycleTime = 0;
    static MIN_CYCLE_INTERVAL = 30_000; // 30s throttle
    // ── Direct Message Wake (trigger loop cycle on direct messages: telegram, room, chat) ──
    directMessageWakeQueue = 0;
    lastDMWake = 0;
    ...
    lastApiRoomTriggerAt = 0;
    static SENTINEL_ECHO_WINDOW = 3_000; // 3s — sentinel debounce is 500ms
    busyRetryCount = 0;
    static DM_WAKE_THROTTLE = 5_000; // 5s throttle for all DM sources
    // ── Cooperative Yield (Layer 3) ──
    pendingPriority = null;
    // ── Interrupt storm guard (Layer 2) ──
    lastPriorityDrainAt = 0;
    static PRIORITY_COOLDOWN = 10_000; // 同類 10s 冷卻
    // ── Continuation Check (mushi System 1 exit actuator) ──
    consecutiveContinuations = 0;
    static MAX_CONSECUTIVE_CONTINUATIONS = 5;
    continuationCooldownUntil = 0; // Unix timestamp — no continuation checks until this time
    lastCycleHadSchedule = false;
    concurrentInboxDetected = false;
    lastCycleMeshQueued = false;
    // =========================================================================
    ...
    static DIRECT_MESSAGE_SOURCES = new Set(['telegram', 'room', 'chat']);
    /** Unified event handler — all inputs through single L0→L4 pipeline */
    handleUnifiedEvent = (agentEvent) => {
        if (!this.running)
            return;
    ...
    handleEventWhileCycling(event, decision, agentEvent, now) {
        ...
        if (AgentLoop.DIRECT_MESSAGE_SOURCES.has(event.source)) {
            const text = agentEvent.data?.text ?? '';
            const roomMsgId = agentEvent.data?.roomMsgId ?? undefined;
            if (text) {
        ...
        switch (decision.lane) {
            ...
            case 'immediate': {
                ...
                if (AgentLoop.DIRECT_MESSAGE_SOURCES.has(event.source) && (this.triggerReason?.startsWith('telegram-user') || this.currentMode === 'idle')) {
                ...
                const msgCount = agentEvent.data?.messageCount ?? 1;
     ...
     */
    async foregroundReply(source, text, replyTo, opts) {
    ...
    activeForegroundHashes = new Set();
    async executeForegroundCall(source, text, replyTo, opts) {
...
3. 不在此 lane 做長時間推理
            ...
            const onStreamChat = (chatText, reply) => {
            ...
            try {
                getLogger().logBehavior('agent', 'action.foreground', `[${source}] ${answer.slice(0, 500)}`);
            }
            catch { /* fire-and-forget */ }
    ...
    handleEventNotCycling(event, decision, agentEvent, now) {
        ...
        // Messages are batched per-source (3s window) to merge rapid-fire messages.
        const messageText = agentEvent.data?.text ?? '';
    ...
    constructor(config = {}) {
        ...
        this.batchBuffer = new BatchBuffer((source, mergedText, replyTo) => this.executeForegroundCall(source, mergedText, replyTo).catch(() => { }), 3000);
        // DelegationBatchBuffer: collect rapid delegation completions into one cycle (10s sliding window)
        this.delegationBatchBuffer = new DelegationBatchBuffer((_taskIds, count) => {
    ...
    start() {
        ...
        try {
            initSharedKnowledge(getInstanceDir(getCurrentInstanceId()));
        }
        catch { /* best effort */ }
        ...
        if (stale) {
            ...
            if (stale.lastAction)
                this.lastAction = stale.lastAction;
        ...
        eventBus.on('action:delegation-complete', (event) => {
            ...
            const delegationType = event?.data?.type;
            if (delegationType === 'learn' || delegationType === 'research') {
                const outputPreview = event?.data?.outputPreview;
                import('./myelin-fleet.js').then(({ triageLearningEvent }) => triageLearningEvent({
                    ...
                    import('./delegation.js').then(({ getTaskResult }) => {
                        ...
                        if (result?.status === 'completed' && result.output) {
                            import('./small-model-research.js').then(({ processResearchResult }) => {
                                processResearchResult(result.output, delegationType, result.duration ?? 0, result.confidence);
                            }).catch(() => { });
                        }
                    }).catch(() => { }); // fire-and-forget
                }
            ...
            try {
                kbObserve({
                    ...
                    outcome: event?.data?.status === 'completed' ? 'success' : 'fail',
    ...
    adjustInterval(hadAction) {
        if (hadAction) {
            ...
            this.currentInterval = Math.min(this.currentInterval * this.config.idleMultiplier, maxInterval);
        }
    ...
    async runCycle() {
        if (!this.running || (this.paused && !this.calmWake))
            return;
        this.calmWake = false;
        ...
        if (isEnabled('mushi-triage') && !isContinuation && (!hasP0 || isDM) && reason) {
            ...
            if (triageSource === 'alert') {
            ...
            else if (this.cycleCount > 1 // Never hard-skip first 2 cycles after restart — prevents idle loop from crash-resumed lastAction
                ...
                try {
                    kbObserve({ source: 'mushi', type: 'skip', data: { trigger: triageSource, reason: 'hard-rule: no-perception-change + idle' }, tags: [triageSource] });
                }
                catch { /* fire-and-forget */ }
                writeTrailEntry({
                ...
                if (decision === 'skip') {
                    ...
                    try {
                        kbObserve({ source: 'mushi', type: 'skip', data: { trigger: triageSource, reason: 'mushi-triage' }, tags: [triageSource] });
                    }
                    catch { /* fire-and-forget */ }
        ...
        if (this.running && !this.paused && isEnabled('mushi-triage') && !this.lastCycleHadSchedule && !this.lastCycleMeshQueued) {
            if (Date.now() < this.continuationCooldownUntil) {
                ...
                const contCtx = { lastAction: this.lastAction, triggerReason: this.triggerReason };
    ...
    async runConcurrentTasks() {
        ...
        tasks.push(perceptionStreams.refreshAll().catch(() => { }));
        // 2. Auto-commit + auto-push (previous cycle's leftover changes)
        if (isEnabled('auto-commit')) {
            tasks.push(autoCommitMemoryFiles(null)
                .then(() => {
                try {
                    getMemory().updateConversationSearchIndex();
                }
                catch { /* best effort */ }
            })
        ...
        try {
            ...
            if (dmItems.length > 0) {
                ...
                try {
                    ...
                    for (const slot of lanes.foreground.slots) {
                        if (slot.task?.prompt) {
                            ...
                            if (senderMatch)
                                activeFgSenders.add(senderMatch[1]);
                ...
                for (const decision of fgItems) {
                    ...
                    if (itemMsgId)
                        claimMessage(itemMsgId, 'foreground');
            ...
            let specialistPerspective = null;
                ...
                try {
                    ...
                    if (fs.existsSync(perspPath)) {
                        ...
                        if (perspective && perspective !== 'primary') {
                            specialistPerspective = perspective;
                        }
                    }
                }
            ...
            if (isTelegramUserCycle) {
                ...
                try {
                    ...
                    const extractMsgId = (line) => line.match(/\[(\d{4}-\d{2}-\d{2}-\d+)\]/)?.[1];
            ...
            if (pendingFgDelegations.length > 0) {
                const fgPreview = pendingFgDelegations.map(d => `  - [${d.source}] "${d.text.slice(0, 120)}" (delegated at ${new Date(d.delegatedAt).toLocaleTimeString()})`).join('\n');
            ...
            const workJournalSuffix = this.workJournalContext
                ...
                : '';
            if (this.workJournalContext)
                this.workJournalContext = null; // one-shot: 用完即清
            ...
            const onPartialOutput = (text) => {
                const now = Date.now();
                if (now - lastCheckpointUpdate < 30_000)
                    return;
            ...
            const streamedChatTexts = new Set();
            const onStreamChat = (text, reply) => {
            ...
            // 結構化記錄 Claude 呼叫
            logger.logClaudeCall({ userMessage: prompt, systemPrompt, context: `[${context.length} chars]`, fullPrompt }, { content: response }, { duration, success: true, mode: this.currentMode });
            // ── Act ──
            ...
            if (streamedChatTexts.size > 0) {
                ...
                tags.chats = tags.chats.filter(c => {
                    if (streamedChatTexts.has(c.text))
                        return false; // exact match
                    if (normalizedStreamed.has(normalize(c.text)))
                        return false; // normalized match
                    return true;
                });
            ...
            for (const del of tags.delegates) {
                ...
                try {
                    kbObserve({ source: 'delegation', type: 'spawn', data: { taskId, taskType, workdir: del.workdir }, tags: [taskType] });
                }
                catch { /* fire-and-forget */ }
            ...
            // ── Write Trail Entry (fire-and-forget, shared attention history) ──
            {
                ...
                const detailParts = [];
                if (currentTriggerReason)
                    detailParts.push(`trigger: ${currentTriggerReason}`);
                if (action)
                    detailParts.push(`decision: ${action.slice(0, 200)}`);
                if (cycleTagsProcessed.length > 0)
                    detailParts.push(`tags: ${cycleTagsProcessed.join(',')}`);
                writeTrailEntry({
                    ts: new Date().toISOString(),
                    agent: 'kuro',
                    type: 'focus',
            ...
            if (isEnabled('auto-commit') && !isEnabled('concurrent-action')) {
                ...
                autoCommitMemoryFiles(action)
                    .then(() => {
                    try {
                        memory.updateConversationSearchIndex();
                    }
                    catch { /* best effort */ }
                })
                    .then(() => autoCommitExternalRepos())
            ...
            if (isEnabled('housekeeping')) {
                ...
                if (this.cycleCount % 10 === 0) {
                    import('./memory-summarizer.js').then(({ runSummarizationCycle }) => runSummarizationCycle(memory.getMemoryDir())).catch(() => { });
                }
            }
            ...
            try {
                const route = buildCycleRoute(currentTriggerReason ?? 'unknown', modelRoute.model, context, action, isVisibleOutput(action), duration);
            ...
            try {
                trackNutrientSignals(action, response);
            }
            catch { /* fire-and-forget */ }
            ...
            try {
                ...
                if (tags.chats.length > 0)
                    outputTags.push('chat');
                if (tags.shows.length > 0)
                    outputTags.push('show');
                if (tags.dones.length > 0)
                    outputTags.push('done');
                if (tags.summaries.length > 0)
                    outputTags.push('summary');
                if (tags.remembers.length > 0)
                    outputTags.push('remember');
                if (tags.delegates.length > 0)
                    outputTags.push('delegate');
                if (tags.archive)
                    outputTags.push('archive');
                if (tags.impulses.length > 0)
                    outputTags.push('impulse');
                if (tags.tasks.length > 0)
                    outputTags.push('task');
                if (tags.asks.length > 0)
                    outputTags.push('ask');
                recordCycleNutrient({
    ...
    computeActionSimilarity(action) {
    ...
    updateDailyMetrics(mode, rememberCount, similarity) {
        this.ensureMetricsDate();
        if (mode === 'task')
            this.dailyTaskCycles++;
        if (mode === 'autonomous')
            this.dailyAutonomousCycles++;
    ...
    isWithinActiveHours() {
        if (!this.config.activeHours)
            return true; // No restriction = 24h active
