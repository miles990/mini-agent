...
export function flushFeedbackState() {
 ...
 */
export async function trackPerceptionCitations(action) {
    if (!action)
        return;
    const state = readState('perception-citations.json', {
        cycleCount: 0,
        citations: {},
        lastAdjusted: '',
    });
 ...
 */
export async function auditDecisionQuality(action, triggerReason, response) {
    if (!action && !response)
        return;
    const state = readState('decision-quality.json', {
        recentScores: [],
        avgScore: 0,
 ...
 */
export async function auditSystemHealth() {
    const state = readState('system-health.json', {
        perceptions: {},
    ...
    for (const s of stats) {
        ...
        if (s.timeouts >= 3) {
            ...
            if (cooldownOk) {
                ...
                await memory.addTask(`P2: 感知健康 — ${s.name} 連續 ${s.timeouts} 次 timeout（avg ${s.avgMs}ms）@due:${dueDate}`);
    ...
    try {
        ...
        if (exists(cdpLog)) {
            ...
            const domainRestrictions = new Map();
 ...
 */
function recordBaselineCycle(action, context, triggerReason, model) {
    if (!context)
        return;
    ...
    let score = 0;
    if (action) {
        if (/##\s*Decision|\[DECISION\]/i.test(action))
            score++;
        if (/##\s*What|\*\*What/i.test(action))
            score++;
        if (/##\s*Why|\*\*Why/i.test(action))
            score++;
        if (/##\s*Thinking|\*\*Thinking/i.test(action))
            score++;
        if (/##\s*Changed|\*\*Changed/i.test(action))
            score++;
        if (/##\s*Verified|\*\*Verified/i.test(action))
            score++;
    }
    const record = {
...
export async function auditStructuralHealth(triggerReason) {
    const state = readState('structural-health.json', {
        cyclesSinceLastCheck: 0,
        lastCheckAt: null,
        lastWarnings: [],
    });
    ...
    try {
        const dqState = readState('decision-quality.json', {
            ...
            if (existsSync(crsPath)) {
                ...
                for (const line of lines) {
                    try {
                        ...
                        if (/^cron|^heartbeat|^workspace/.test(trigger))
                            lightCycles++;
                    }
                    catch { /* skip */ }
                }
    ...
    try {
        ...
        const recentSrcChanges = execSync('git log --since="24 hours ago" --oneline -- src/ 2>/dev/null | head -5', { encoding: 'utf-8', timeout: 5000 }).trim();
 ...
 */
export async function trackCompoundInterest(cycleCount) {
    const state = readState('compound-scores.json', {
        lastScanCycle: 0,
        referencedBy: {},
        scores: {},
        topCompound: [],
    });
    if (cycleCount - state.lastScanCycle < COMPOUND_SCAN_INTERVAL)
        return;
    ...
    const topicNames = topicFiles.map((f) => f.replace('.md', ''));
    // Read all topic contents
    const topicContents = {};
    for (const file of topicFiles) {
        try {
            topicContents[file.replace('.md', '')] = readFileSync(path.join(topicsDir, file), 'utf-8').toLowerCase();
        }
...
export function getCompoundScores() {
    const state = readState('compound-scores.json', {
 ...
 * 每 10 cycles 執行。
 */
export async function auditProblemAlignment(action) {
    const state = readState('problem-alignment.json', {
        cyclesSinceLastCheck: 0,
        lastCheckAt: null,
        alignmentHistory: [],
        warningActive: false,
        ...
        if (existsSync(crsPath)) {
            ...
            for (const line of lines) {
                try {
                    const record = JSON.parse(line);
                    if (record.citations)
                        actionText += ' ' + record.citations.join(' ');
                    if (record.triggerReason)
                        actionText += ' ' + record.triggerReason;
                }
                catch { /* skip */ }
            }
        }
...
// Unified Entry Point
// =============================================================================
export async function runFeedbackLoops(action, triggerReason, context, cycleCount, model, response) {
    ...
    if (cycleCount)
        await trackCompoundInterest(cycleCount).catch(() => { });
    ...
    if (cycleCount && cycleCount % 50 === 0) {
        try {
            const { scanCapabilityGaps } = await import('./evolution.js');
            await scanCapabilityGaps();
        }
        catch { /* ignore */ }
    }
    ...
    if (cycleCount && cycleCount % 50 === 0) {
        try {
            const { applyRoutePruning } = await import('./route-tracker.js');
            applyRoutePruning();
        }
        catch { /* ignore */ }
    }
    ...
    try {
        recordBaselineCycle(action, context ?? null, triggerReason, model);
    }
    catch { /* ignore */ }
    // Achievement system (fire-and-forget)
    try {
        const { checkAchievements } = await import('./achievements.js');
        await checkAchievements(action, cycleCount ?? 0);
    }
    catch { /* ignore */ }
}
//# sourceMappingURL=feedback-loops.js.map