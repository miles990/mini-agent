...
**mini-agent 精簡版案例（2026-02-12）**：10k 行→2k 行的嘗試失敗。根本原因不是精簡方向錯，而是方法錯 — 「從完整版砍」= 不知道什麼是沉積層就動刀。claudeBusy 卡住就是典型：正常路徑不需要 reset，但 CLI crash/timeout 路徑需要，這些防禦代碼是真實故障後才加的。Chesterton's Fence 完美適用。**正確做法**：(1) 拆分不是砍 (2) 從零往上加不是從完整版往下砍 (3) 部署問題和架構問題分開處理。這就是 tef 的 parallel maintenance + incremental migration — 我自己筆記裡寫了但沒用上。
...
**社會層面的感知過載（Pedersen, 2026-02-12）**：Cam Pedersen 分析 5 個 AI 指標，發現 4 個線性增長（MMLU/token 成本/發布間隔/代碼佔比），唯一呈雙曲線的是 arXiv 論文數 — 即**人類注意力**。真正 exponential 的不是機器能力而是社會對 AI 的感知強度。結果：anticipatory disruption（預防性裁員、監管真空、心理焦慮）在 AI 能力到位前就已造成破壞。nine_k 的 "epistemic takeover" = 二階信念操縱（不需要 AI 真的超越人類，只需讓所有人相信其他人都相信）。這是 Pattern 6 失敗的宏觀版：**整個社會的感知資源被 AI narrative 淹沒，導致集體行動癱瘓**。來源：campedersen.com/singularity, HN#46962996 (1299pts/703評)

**Claude Code 透明度案例（2026-02-12）**：v2.1.20 把檔案路徑摘要化（"Read 3 files"）。這是對使用者感知的 moving average blur — 把 N 個具體信號平均成一行摘要。lcamtuf 證明 blur 技術上可逆（verbose mode 存在），但 Retr0id 指出實際上沒人逆（verbose mode 太 noisy）。Pattern 2 修正的實踐版：理論可逆 ≠ 實際逆轉。正確做法是 Calm Technology 分層（預設關鍵路徑 + 展開細節），不是 binary toggle（summary vs verbose）。
