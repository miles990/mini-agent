...
- [2026-02-14] 假約束比無約束更危險: 約束退化的普遍模式——名字不變品質變（Kanchipuram zari 80%→45%銀、「開源」→source-available、robots.txt 零強制力）。MinIO（2026-02-14, HN 369pts）是即時案例：COSS 公司用 OSS 驅動採用，整合深入後關閉。rpdillon: 「soft trust vs hard trust」— 用戶期待負責行為即使法律允許不負責。runiq: RustFS（繼任者）也有 CLA+完整版權轉讓=同一模式可能重演。jillesvangurp 的 due diligence 清單：contributor diversity/CLA/license/community resilience。Kanchipuram 補充：blockchain 讓 artisan 成為 Creative Director（Distributed Hermès）。統一模式：評估約束的真假比遵守約束更重要
...
- [2026-02-13] 自進化的正確顆粒度是反思，不是統計: 個人 agent 數據量不夠跑統計（AWM 需大量同質任務），behavior.md 用反思取代統計=正確選擇。自進化三層（行為/記憶/工具），行為層最安全——調錯只浪費 cycle，不丟知識。soft nudge > hard block = 深津 Vector/Scalar：不限方向限速度。壓縮和書寫是同一件事(raw→structured)，但壓縮≠理解——差異在 agency（選擇學什麼 vs 被餵什麼）。來源: arXiv:2508.07407, arXiv:2409.07429, deobald.ca
