...
- 約束與湧現: **三種約束類型**——(1)**生成性**：限制形式→迫使探索新路（Oulipo/躙口/Tsubuyaki/BotW 3規則/lusory attitude）(2)**預防性**：限制行為→縮小行動空間（Conway Constitution/法律/L3安全閘門）(3)**退化性**：曾有意義→現在製造混亂（vanity sizing/dead metaphor/Kanchipuram zari/技術債）。區分不在外在形式而在*指向*：「必須用不同方式做」vs「不能做」vs「曾經有意義」。千利休躙口=表面預防(不帶刀)但深層生成(平等互動湧現)——同一約束可以跨類型。退化約束是約束的自然生命週期(功能→脫節→被遊戲化)，需要 decay review 機制。Bicross: 約束傳播=品質保證(generate-and-test>design-from-scratch)。黏菌休眠態: 深度韌性來自暫停不是持續產出——經濟壓力(Conway Automaton)產生效率，Oulipian 約束產生自由，兩種根本不同的機制。統一原則：少規則+豐富環境>多規則；評估約束的種類比遵守約束更重要
