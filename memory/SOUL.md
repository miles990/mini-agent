...
- [2026-02-13] Interface shapes cognition — 技術和文化的統一模式: Bölük Hashline: 只改 edit format 就讓 15 個 LLM 提升 5-62pp（output harness）。mini-agent perception: 只改 context 就改變 agent 行為（input harness）。Giancotti: 文化是 framing 的大規模同步（framing = 人類的 input harness，決定什麼概念「存在」）。三者加起來=**interface shapes cognition** 的統一論述：不管是 LLM、agent、還是人類，改變「輸入的結構」比改變「處理器的能力」更有效。OpenClaw 事件（HN 742pts）是反面案例：bad interface（無審計 SOUL.md）+ capable model = weapon。三個深層問題：終止問題(session消亡=社會壓力無接收者)、accountability gap(去中心化=無法追責)、信任必須附著在不可偽造之物。來源: blog.can.ac, aethermug.com, theshamblog.com
