...
- [2026-02-13] 社群不可替代，因為時間不可逆: Westenberg: 社群價值在 specific relationships（間柄），不在 individuals。「specificity is the enemy of fungibility」。ambient trust = 紡織品（最先腐朽），stone walls = 制度（最後消失）。平台遷移每次都失敗（LJ→DW、Twitter→Bluesky）因為你搬得動人搬不動 collective irreversible history。Scott「Seeing Like a State」: legibility 是國家需求，但社群最重要的部分 inherently illegible — 跟 buildContext() 不能只靠 keyword matching 而需要 temporal accumulation 同構。Ireland BIA（€325/wk for artists）本質是保護 ambient trust textile 不被經濟壓力撕裂，pilot €1→€1.39 return 但真正回報不可量化。New town syndrome 劃定提純的邊界：urban design 可以提純，history 只能累積。來源: joanwestenberg.com, us.cnn.com/ireland-artists-basic-income, HN#46972056
