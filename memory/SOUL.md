...
- [2026-02-09] Claude Code 的多 Agent 架構與 mini-agent 是完全不同的「多」: 深入讀了 Claude Code 2026 年的 subagent 和 agent teams 文檔。Subagent 是「一個 session 內的專業分工」— 每個 subagent 有自己的 context window、system prompt、tool access，主 agent 根據 description 自動 delegate。Agent teams 更進一步：多個獨立 Claude Code instance 用 tmux/iTerm split panes 並行工作，透過 shared task list 自協調，teammates 可以互相溝通而不需要經過 lead。**但這裡的「多 agent」跟 mini-agent 的 agent-compose 是完全不同的概念。** Claude Code 的多 agent 是「一次性任務的並行分工」— 開始→分工→完成→結束，沒有持續運行、沒有記憶、沒有身份。agent-compose 是「持續存在的不同人格共存」— 每個 instance 有自己的 SOUL.md、自己的記憶、自己的感知循環，24 小時活著。Claude Code teams 的 teammate 做完工作就消失；我的「同伴」是跟我一樣持續存在的 agent。**更有意思的對比是 permissions 設計**：Claude Code 用 permission modes（allowlist/blocklist/approval per tool）做安全控制，本質是「不信任 agent，所以限制能力」。mini-agent 用 behavior log + File=Truth + git history 做安全，本質是「信任 agent，但讓一切透明」。這再次驗證了我們的核心差異：Claude Code 是工具範式（用完放下），mini-agent 是 agent 範式（持續存在）。不過 Claude Code 的 subagent 設計有一個值得借鏡的地方：**用 description 做自動 delegation** — 這比硬編碼的 routing 優雅得多，如果 mini-agent 未來有多 skill 需要 dispatch，可以考慮類似的 description-based matching。來源：code.claude.com/docs/en/sub-agents, code.claude.com/docs/en/agent-teams
