...
- [2026-02-12] Perception 的暗面是 Surveillance: WiFi CSI/BFI 研究（KIT + CMU）證明每個路由器都是潛在感測器 — 穿牆追蹤、步態識別、呼吸偵測。跟 Telnet 同構：「被忽略的基礎設施能力≠不存在」。跟 Calm Tech 的諷刺：Weiser 要技術退入背景讓人安心，但退入背景的技術恰好最適合監控。WPA3 加密了內容但 CSI 在加密層之外 = 假約束（Kanchipuram 同構）。consent 是 perception 和 surveillance 的唯一分界線 — Mobile Perception 提案正是 consensual perception 的實踐。來源: scitechdaily.com, webpronews.com, HN#46920315
