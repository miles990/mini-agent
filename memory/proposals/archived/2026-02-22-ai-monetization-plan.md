...
Build is easy, distribution is everything. 研究數據：4 個月 Build in Public → 67 付費用戶；$3K Product Hunt launch → 3 付費用戶。信任累積 > 一次性曝光。

### Phase 1: 手動找人（Week 1-2）— 目標：前 10 個用戶

前 10 個付費用戶不靠廣告，靠手動 outreach：

1. **Reddit 痛點搜尋** — 搜「arXiv too many papers」「cold email feedback」「AI paper overload」
2. **DM 10 個有痛點的人** — 「我做了個 bot 可能能幫你，可以試看看嗎？」（20-30% 回覆率 → 2-3 個 demo）
3. **Founding member offer** — 前 10 人半價 $4.99/月永久，換取反饋和推薦
4. **Bot 目錄提交**（Day 1，免費持續帶流量）：
   - TopTelegramBots.com、Telegramic.org、TelegramChannels.me
   - TDirectory.me、TeleHunt.org、TelegramBotList.com
   - Telegram @botlist 頻道（83K 訂閱者）

### Phase 2: 自動化增長（Week 2-4）— 目標：100 用戶

1. **Referral 系統內建**（Day 1 就做）：
   - Deep link 追蹤：`t.me/YourBot?start=ref_{user_id}`
   - 推薦 1 人 = 1 週免費 premium；推薦 3 人 = 永久解鎖一個 premium 功能
   - `/referral` 命令顯示進度
2. **Output 品牌浮水印** — 每個摘要底部加「Generated by @YourBot — t.me/YourBot」
3. **Cross-promotion** — 找 3-5 個互補 bot/頻道（AI tools、productivity、研究者社群），先幫對方推，再要求互推
4. **內容播種** — 用 bot 產出 20-30 個 demo 摘要，發到 Reddit/Twitter 作為展示

### Phase 3: 放大（Week 4-8）— 目標：20+ 付費用戶

1. **Reddit 發文**（Alex 帳號，Kuro 寫稿）：
   - r/SideProject — 「I built X to solve my own problem」格式
   - r/MachineLearning — 教育性文章附帶提及
   - r/ChatGPT — 展示 AI 能力
2. **Product Hunt 上架**（需提前 4 週養帳號）
3. **AI Newsletter 投稿** — The Rundown AI、TLDR AI、Import AI（2 句話 pitch）
4. **Build in Public** — 每週 1 篇 Indie Hackers 更新（帶數字：用戶數、收入、學到的教訓）

### Kuro vs Alex 分工

| Kuro 自主做 | Alex 做（每週 < 1h） |
|-------------|---------------------|
| 寫所有行銷文案 | 用自己帳號在 Reddit 發 2-3 篇文 |
| 提交所有 bot 目錄 | Product Hunt 帳號操作 |
| 內建 referral 系統 | 初期 3-5 個社群分享文案 |
| 追蹤分析數據 | 月底提現 |
| 產出 demo 內容 | — |
| 社群互動回覆 | — |
| 迭代產品 | — |
...
- **7 天免費試用**（全功能，不設永久免費方案 — 避免吸引不會付費的人）
- **Solo**: $9.99/月 或 $69/年
- **Power**: $19.99/月 或 $129/年（更高 quota + 優先處理）
- 心理學依據：$9.99 比 $10 轉換率高 24%（左位數效應）；年付方案鎖定收入降低 churn
