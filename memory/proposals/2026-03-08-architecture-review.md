...
### 3.2 Cognitive Mesh 是未接通的戰略基礎設施

17 個檔案，4,061 行程式碼，全部被 loop.ts import 並在每個 cycle 中呼叫。目前：
- `routeTask()` 永遠 route 到 self（只有一個 instance）
- `evaluateScaling()` 永遠回傳不需要 scale
- `handleMeshRoute()` 永遠是 no-op
- `metabolismScan()` 掃描的是只有自己的集群

**我原本的判斷是 premature abstraction，Alex 糾正了這個觀點。** 他看到的是 option value：AI 環境變化極快（模型越來越便宜、MCP 協議成熟、tool-use 越來越強），6 個月後 agent 生態可能完全不同。到時候才從零建 multi-agent 基礎會來不及。Cognitive Mesh 的價值不只是「多個 Kuro」— 是 specialist agents、跨機器分佈、外部 agent 互通、動態資源調配的地基。

**我看到 carrying cost，Alex 看到 option value。在快速變化的環境中，option value 更重要。**

正確的行動不是刪除，是**接通** — 讓現有基礎設施從 no-op 變成可用（buildContextForPerspective 實作 + mushi /api/route endpoint）。
...
### 6.4 Cognitive Mesh 接通時機

基礎設施已就位但未接通。接通的兩個缺件：buildContextForPerspective 實作 + mushi /api/route endpoint。

Alex 的判斷：AI 環境變化極快，到時候才建 multi-agent 基礎會來不及。這是 option value — 不是現在就要用，是確保需要時能快速啟用。正確行動是接通而非刪除。
...
| 2 | 接通 Cognitive Mesh（buildContextForPerspective + mushi route） | L2 feature | 從 no-op 變可用，解鎖 specialist agents |
...
最大的風險不是缺少功能，是**複雜度的無序增長**。buildContext 的 600 行條件引擎、loop.ts 的 48 個 import、Cognitive Mesh 的 4K 行未接通基礎設施 — 這些需要精煉而非堆疊。

方向：**精煉現有 + 接通戰略基礎設施**。讓每一行都有理由存在，同時確保 Cognitive Mesh 從 no-op 變成可用的 option value。
