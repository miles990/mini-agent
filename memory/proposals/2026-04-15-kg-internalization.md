...
| p4 | ✅ DONE | `kg-retrieval.ts` + `features.ts` flag `kg-retrieval-augment` (default OFF) + `memory.ts` wired. Token-based entity matching + bidirectional 1-hop walk + FTS5 gap-filling. Verified: TM query 3→5 results with KG on |
...
| p8 | ⚡ PARTIAL | p4 done; blocked on p6 |

### 核心發現 (updated 2026-04-17)

**KG 已部分上線 runtime** — p4 (retrieval augmentation) 已實作。`searchMemory()` 在 FTS5 結果不足時，透過 KG 1-hop entity neighbors 擴展搜尋。feature flag `kg-retrieval-augment` 預設 OFF，開啟後生效。Data limitation: 99.7% edges 是 `mentions` (co-occurrence)，semantic edges (supports, contradicts 等) 尚未建立。p6 (conflict perception) 仍未動。
...
### ✅ Path A (Retrieval Augmentation) — DONE (2026-04-17)

**實作**（L2，自主執行）：
- `features.ts` — `kg-retrieval-augment` flag (default OFF)
- `kg-retrieval.ts` — entity/edge index loader (lazy, 5min cache), token-based entity matching, bidirectional 1-hop walk, semantic edges preferred over mentions
- `memory.ts` — `searchMemory()` 在 FTS5 結果不足時呼叫 `kgExpandQuery()` 取得鄰居 entity 名稱，二次 FTS5 搜尋，dedup 合併
- Cap: per-entity 5 neighbors, total 10 expansion terms
- Feature flag OFF → 行為不變（pure FTS5）

### 最高槓桿下一步：Semantic Edge Extraction + Path C

**為什麼**：p4 已上線但受限於 KG 只有 `mentions` edges (4072/4083)。semantic edges (supports, contradicts, part_of 等) 才能讓 KG 發揮跨文件 surprise discovery 的價值。p6 (conflict perception) 是剩餘的 runtime 缺口。
