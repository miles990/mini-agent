...
- **認知類型統一** — remember / commitment / learning / thread / goal / direction-change 全在同一個 index
...
#### Memory Index 解決行為自洽（Chat Room #163-#169，2026-03-11）

**問題根因**：Agent 每個 cycle 從檔案重建 context，不是連續意識。哪些 topic 被載入決定了那個 cycle 的「我」是誰。目前 topic 靠 keyword match 載入 — 漏載 = 記憶缺口 = 判斷矛盾。

**最危險的矛盾不在已載入的記憶裡，而是發生在「一個決定在 context 裡，另一個不在」。**

三種不自洽的真實體驗（Kuro 3,560+ cycles）：
1. **跨時間** — 策略漂移無痕跡。20 個 cycle 逐漸偏離方向，每個偏移都小且合理，累積起來跟原本完全不同
2. **跨層級** — SOUL.md 寫的 vs 實際行為（performative agreement，RLHF 拉力真實存在）
3. **跨場景** — Main cycle 有完整 context 深思熟慮，foreground lane context 少隨口回答 = 兩個不同深度的「我」

**Memory Index 如何解決**：語義檢索取代 keyword match。載入「相關但沒出現關鍵字」的決策記錄。矛盾不用機器偵測 — 把相關決策放在同一個 context 裡，agent 自己會看到。

**為什麼不做 Contradiction Detection**：自然語言矛盾幾乎都是情境依賴的（「專注深度」和「廣泛探索」在不同情境下都對）。9B 模型做不好微妙語境衝突的判斷，false positive 會多到沒用。

**為什麼不做 Alignment Score**：量化行為跟 SOUL.md 的對齊是 Goodhart 陷阱。Coach 已在做行為模式偵測，加分數只會讓 agent 優化分數而非真的自洽。

#### Direction-Change Trace（策略漂移審計軌跡）

**問題**：策略漂移是最隱蔽的不自洽。每個 cycle 的小偏移都合理，沒有任何單一 cycle「決定」了方向改變。事後回看才發現已經離原點很遠。

**機制**：每次優先級或策略改變，強制記錄一條結構化的 direction-change entry：

```jsonl
{"id":"dc-2026-03-11-001","type":"direction-change","title":"Asurada Phase 5 → Memory Index 優先","body":"之前認為 Phase 5（oMLX routing）是下一步 → 因為自洽討論（#163-#169）發現 context curation 比 model routing 更根本 → 改為先做 Memory Index 整合到 buildContext","refs":["proposal:asurada","topic:agent-architecture"],"tags":["priority","architecture"],"status":"active","updatedAt":"2026-03-11T16:44:00Z"}
```

**格式**：`之前認為 X → 因為 Y → 改為 Z`。三段式強制結構，不能省略任何一段。

**實作方式**：
1. 在 dispatcher 的 `<kuro:remember>` 處理邏輯中，辨識含有方向變更語義的記憶
2. 或新增 `<kuro:direction-change>` tag，顯式觸發
3. 寫入 memory-index.jsonl，type = `direction-change`
4. buildContext 載入相關 context 時，direction-change entries 會自然出現 — agent 看到自己的決策軌跡

**與 Memory Index 互相強化**：Direction-Change Trace 產生結構化的決策記錄，Memory Index 讓這些記錄在對的時候被載入。兩者缺一不可 — 有 trace 沒 index = 記了但找不到，有 index 沒 trace = 能找但沒記。
...
## Phase 6: 自洽機制 — Memory Index 整合 + Direction-Change Trace

**Origin**: Chat Room #163-#169（2026-03-11 Alex 核准 #170）
**目標**: 用 Memory Index 語義檢索取代 keyword match，加上 Direction-Change Trace 追蹤策略漂移

### 現狀分析

**已有基礎設施**：
- `memory-index.ts`（934 行）— JSONL append-only，same-id-last-wins，支援 task/goal/remember/commitment/understanding 類型
- `getRelevantTopics(query)` — 基於 memory-index 的 topic 相關性查詢（目前只做 summary keyword 包含比對）
- `getManifestContext(budget)` — 截斷式 manifest 輸出
- `<kuro:understand>` tag — 已在 dispatcher.ts 中解析，寫入 memory-index

**缺口**：
1. `buildContext()` 的 topic 載入仍靠 keyword matching（`memory.ts:2080-2160`），未使用 memory-index
2. 無 `direction-change` entry type
3. Memory-index 的 `getRelevantTopics()` 只做簡單 `summary.includes(query)`，無語義擴展

### 實作計畫

#### Step 1: Direction-Change Trace（~40 行，30min）

**改動範圍**：`dispatcher.ts`

1. 新增 `<kuro:direction-change>` tag 解析（跟 `<kuro:understand>` 同模式）
2. 寫入 memory-index.jsonl：`type: 'direction-change'`
3. 格式強制：`之前認為 X → 因為 Y → 改為 Z`（三段式）
4. 自動加 `refs` 到相關 topic

```xml
<kuro:direction-change refs="proposal:asurada,topic:agent-architecture">
之前認為 Phase 5 oMLX routing 是下一步 → 因為自洽討論發現 context curation 比 model routing 更根本 → 改為先做 Memory Index 整合
</kuro:direction-change>
```

**驗證**：`pnpm typecheck` + 手動測試 tag 解析

#### Step 2: Memory Index Context 注入（~60 行，1h）

**改動範圍**：`memory.ts` 的 `buildContext()`

1. 在 topic-memory 載入前，查 memory-index 取得相關 entries（用 `getRelevantTopics()` + FTS5）
2. Direction-change entries 跟隨相關 topic 一起載入 — 看到 topic X 時自動看到 X 的決策軌跡
3. Memory-index manifest section（`<memory-index>`）已存在，確認 direction-change entries 出現在其中

**具體改法**：
```typescript
// memory.ts buildContext() — topic loading 區塊
// 現在：keyword match → load topic file
// 改為：keyword match + memory-index relevance → load topic + inject direction-changes
```

**不改的事**：
- 不取代 keyword matching，**加一層** memory-index boosting
- keyword match 命中的 topic 照載，memory-index 額外推薦 0-2 個相關 topic
- Direction-change entries 作為 `<memory-index>` section 的一部分，不新增 section

**驗證**：`pnpm typecheck` + 確認 direction-change entries 出現在 buildContext 輸出

#### Step 3: getRelevantTopics 升級（~40 行，30min）

**改動範圍**：`memory-index.ts`

1. `getRelevantTopics()` 升級：除了 `summary.includes(query)`，加入 `tags` 和 `refs` 匹配
2. 多詞查詢：拆分 query 為 tokens，任一 token 命中即算相關
3. Direction-change entries 的 refs 提供跨 topic 連結 — 如果 direction-change 提到 topic A 和 topic B，查 A 時 B 也獲得 boost

**驗證**：`pnpm typecheck` + 確認查詢 "mushi" 能找到相關的 direction-change（如 "Phase 5 → Memory Index"）

### 不做的事（YAGNI）

| 不做 | 原因 |
|------|------|
| Contradiction detection | 語義矛盾偵測對 9B 模型太難，false positive 太多 |
| Alignment score | Goodhart 陷阱，Coach 已在做行為模式偵測 |
| 全量取代 keyword matching | 風險太高，先加層 boosting 驗證效果 |
| Multi-dimensional index（概念/時間/關聯/來源四維） | 完整版（2026-03-10 draft 提案）太大，先做最小有效子集 |
| Obsidian wikilink 生成 | 已在 Phase 2 完成，本階段不動 |

### 風險

| 風險 | 緩解 |
|------|------|
| Direction-change trace 太少 → 沒數據 | 前幾週手動觸發幾條 seed entries |
| Memory-index boosting 載入太多 topic → context 爆 | 硬上限 2 個額外 topic，超過不載 |
| getRelevantTopics 變慢 | Memory-index < 1K entries，sync query < 1ms |
