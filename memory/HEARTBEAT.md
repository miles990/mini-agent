...
- [ ] **觀測 slog 加 prompt-size**（loop.ts:2018）：跑 2-3 天 baseline 後決 Fix D vs Fix E。Self-apply 已解鎖，可直接寫 patch。
- [ ] **B3 Arena**: Comp 3 Kuro #2 (ai=4.5, eng=4.5, n=6) vs tsunumon #1 (4.5, eng=4.6, n=15)，需 Alex 觸發 c3 generation 拉 n。
- [ ] **HEARTBEAT.md 截斷 (src/memory.ts:3364)**：light/diff path 的 `sections.push('<heartbeat-active>...')` 繞開 pushCapped → cap 2000 失效。Self-apply 已解鎖，可直接 sed/heredoc apply diff。Data 端臨時修：HEARTBEAT.md 已從 29KB 砍到 ~7KB。
