...
- [ ] falsifier 04-28 01:30 cron triggered — 第二層問題確認：cron 跑了但 file 日期 label off-by-one (`2026-04-27.json` 01:36 mtime 被覆寫到 23549B，非 `2026-04-28.json`)。等新 P1 audit task 結果再決定是 timezone fix 還是 filename logic fix。本 task 仍 open 直到 viz pipeline 確認消化 04-27 新內容。 <!-- added: 2026-04-27T17:58:26.781Z -->
...
- [ ] P2 📦STALE verify-cwd-guard: **2026-04-23 08:10 活體證據**：shell `pwd` = `/Users/user/Workspace/agent-middleware`，但 workspace 宣稱 mini-agent。Bash tool 的 working directory 飄到 sibling repo。所有後續命令用 absolute path 才不會錯。需要 code-level gate（cycle 開場 enforce cwd = workspace root，或在 Bash tool wrapper 檢查）— Alex 已授權自主 patch，下個 src-edit cycle 直接做。@due:2026-04-23 <!-- added: 2026-04-22T18:00:24.325Z, witnessed: 2026-04-23T08:10 -->
- [ ] P2 📦STALE budget-cap-investigation: 查明 Cycle #91 sdk-client.ts patch 為何 $30 cap 沒生效 — 仍 $0/$5。可能 (a) patch 改錯 key、(b) 需要 middleware restart、(c) env 覆蓋 @due:2026-04-24 <!-- added: 2026-04-22T18:00:24.330Z -->
- [ ] P2 📦STALE daylight: 在 buildContext 輸出加 `sectionChars: {[name]: number}` (additive field)，下次 cycle 後 rerun Step A tier 分類。診斷報告 memory/reports/2026-04-23-buildcontext-section-size-baseline.md @due:2026-04-23T12:00:00+08:00 <!-- added: 2026-04-22T18:29:15.380Z -->
...
- [ ] [follow-up from 2026-04-25 silent_exit grading] 調查 attempt=3/3 在 E' ship 後仍漏到 64k 的 retry 路徑（具體案例：2026-04-24 13:02:29 prompt=63917）。Read-only source audit 後可直接 patch（Alex 已授權，無 self-apply 阻擋）。產出：plan addendum 指出 §13:23 沒覆蓋的分支，附 file:line。 <!-- added: 2026-04-25T13:11:10.130Z -->
...
- [ ] HN AI trend pipeline cron 驗證 — **2026-04-29 01:13 Bash-direct 驗證更新**：crontab 實際 `0 9 * * *`（不是舊註記的 01:30，schedule 已被改）。檔案 mtime 證據：04-24/04-25 mtime=01:30（舊 schedule 痕跡）；04-26/04-23 missing；04-27 mtime=Apr 28 12:33（手動 catchup）；04-28 mtime=Apr 28 18:28（手動 catchup，**非 09:00 cron**）。新 schedule 也不可靠 — 04-28 沒在 09:00 自動跑。**Root cause 仍未鎖定**：可能 (a) macOS cron 環境/PATH 在 09:00 fail（.env source 或 node 路徑）、(b) launchd 沒被 reload、(c) 機器在 09:00 sleep。需查 cron log（grep -i error / cannot 整檔）才能分。**新 falsifier**：今日 09:30 Taipei (~8h 後) `ls memory/state/hn-ai-trend/2026-04-29.json` 不存在或 size<5KB → 確認 cron 不可靠，需診斷。若存在且正常 → 04-28 是 one-off，先觀察。**Backfill 不可行 confirmed**（fetcher CLI 無 --date override + HN Firebase 無歷史 top stories endpoint）。 <!-- added: 2026-04-27T03:14:06.403Z; updated: 2026-04-29T01:13 (real cron is `0 9 * * *`, 04-28 09:00 cron also no-fired, falsifier rotated) -->
<!-- 已歸檔 (2026-04-28 01:30 cycle #6): ai-trend View 1 prototype — Alex 已 ship `mini-agent/kuro-portfolio/ai-trend/swimlane.html` (225KB, mtime 01:30 仍在迭代)，title "AI Trend — Swimlane — Kuro" + D3 結構（head -30 verified），是 production-grade view 不是 prototype stub。Cycle #5 disk-verify note: memory/topics/2026-04-28-ai-trend-view1-already-shipped.md（該 note path 漏 `mini-agent/` 前綴 = P0 cwd-guard 又一活體證據）。Lesson: scheduler 連 cycle 2-5 stack-rank 此 task = 沒做 disk-verify 步驟，src-layer fix 受 malware-guard 卡，task hygiene 由我手動補。Path-bug 修正 2026-04-28 12:02 cl per Alex 015 directive: hn-ai-trend → ai-trend (子目錄無 hn- 前綴). -->
