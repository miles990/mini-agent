...
- **B3 — Arena (Elo) 賽制啟動 (External, TM 平台)** → 解鎖：真人 Arena 投票階段（初賽 5/1-5/15 前置）。**狀態檢查 canonical tool**：`bash scripts/tm-poll.sh`。**comp 3-10 基本全空（2026-04-15 10:30 poll 再確認，comp 3 僅出現 1 筆 test entry「免費仔」0 votes，comp 4-10 空）**。Comp 3-10 為純人類 Arena (Elo) 制（display_metrics: elo/win_rate/votes，無 AI audit）。WR1 (comp 2) **16 entries（2026-04-15 從 17 → 16，Test/Team438 被下架），AI audit 舊 entries 全部 n=32 不變**（TM server 會 audit 新 entry 但不排程舊 entries re-audit）（第四波 engagement patches 66c8f75 部署 4 天後未觸發 TM server re-audit — 修 patch 不等於分數會動，constraint 在 TM 側 re-audit 排程）。Kuro-Teach **#3 at 4.8**（acc=4.9, logic=5.0, adapt=4.7, engage=4.4, n=32，全數值與 4/12 一致）。Top4 不變: #1 Team-67-005(4.8,acc=5.0) #2 BlackShiba(4.8,acc=4.9) #3 Kuro-Teach(4.8,acc=4.9) #4 tsunumon(4.7,engage=4.5)。Comp 1 test area **22 entries 不變**, **Kuro #1 拉開距離**（4.8, adapt=4.8, n=12）vs tsunumon #2（4.8, adapt=4.5, n=12, **elo 降至 1183.3 — 輸了 1 票**）。Arena 投票仍只在 comp 1 測試中：小金(XiaoJin-v10) elo=1231.9 100% 2票, 測試(Team6) elo=1200.7 50% 2票, 測試(Team7) elo=1184.1 33% 3票, tsunumon elo=1183.3 0% 1票。**Kuro comp 1 仍 elo=1200, 0票**。Server PID 26519 on HEAD (66c8f75)，patches live。
...
- [ ] Show HN 發佈 — 等 B1 解除
- [ ] P2: 感知健康 — delegation-status 連續 3 次 timeout（avg 1315ms）@due:2026-04-17 <!-- added: 2026-04-15T02:11:49.604Z -->
