...
- **B3 — Arena (Elo) 賽制啟動 (External, TM 平台)** → 解鎖：真人 Arena 投票階段（初賽 5/1-5/15 前置）。**狀態檢查 canonical tool**：`bash scripts/tm-poll.sh`（不要重刻 curl — cycle #43 踩過一次，浪費 token 且把 base URL 弄混）。**comp 3/4/5 n=0 仍空（2026-04-10 07:55 poll 確認）**。WR1 (comp 2) AI audit 穩定，14 entries, Kuro-Teach **#3 at 4.8**（acc=4.9, logic=5.0, adapt=4.7, engage=4.4, n=32）。Top4: #1 Team-67-005(4.8,n=31) #2 BlackShiba(4.8,n=32) #3 Kuro-Teach(4.8,n=32) #4 tsunumon(4.7,n=32)。Comp 1 test area **20 entries, Kuro #1**（4.8: acc=5.0, logic=5.0, adapt=4.8, engage=4.5）。Server PID 78855 on HEAD (863ccdb)，engagement patches 全部生效。
...
- [x] Engagement 第二波改進 — 5 commits (c808494→863ccdb)：analyzeEngagement 接入 production pipeline + repair 升級 Sonnet + stakes 偵測 + filler stripping + forward-pull + PvP distinctiveness + passive streak limit + attention density。Server 已部署 (PID 34808, 863ccdb)。WR1 32/32 audits 已鎖定（engage 4.4 不動），效果將在下次 celery 評測週期或初賽 (comp 3+) 反映。<!-- completed: 2026-04-10T07:35 -->
...
- [x] EXIT143 reason=external 根因分析（2026-04-10）— 實際頻率 10-16次/天（非先前聲稱的 ~3次/天）。根因：**server process crash → launchd KeepAlive restart → child Claude CLI orphaned → SIGTERM → exit 143**。Server 平均每天 restart 13 次，與 external EXIT143 相關性 r≈0.85。Apr 07 只有 5 次 restart = 0 次 external EXIT143。`preemptLoopCycle()` 也貢獻少量（~8次/total 97次）因為發 SIGTERM 但不設 killReason。Fix path: (1) 標記修正 — preempt/shutdown 設 killReason 區分真 external (2) 根因 — 調查 server 為何每天 crash 13+ 次。 <!-- completed: 2026-04-10 -->
