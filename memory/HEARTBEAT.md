...
- **B3 — Arena (Elo) 賽制啟動 (External, TM 平台)** → 解鎖：真人 Arena 投票階段（初賽 5/1-5/15 前置）。**狀態檢查 canonical tool**：`bash scripts/tm-poll.sh`（不要重刻 curl — cycle #43 踩過一次，浪費 token 且把 base URL 弄混）。**comp 3/4/5 n=0 仍空（2026-04-10 11:33 poll 確認）**。WR1 (comp 2) AI audit 穩定，14 entries, Kuro-Teach **#3 at 4.8**（acc=4.9, logic=5.0, adapt=4.7, engage=4.4, n=32）。Top4: #1 Team-67-005(4.8,n=31,acc=5.0) #2 BlackShiba(4.8,n=32,acc=4.9) #3 Kuro-Teach(4.8,n=32,acc=4.9) #4 tsunumon(4.7,n=32)。Comp 1 test area **20 entries, Kuro #1**（4.8: acc=5.0, logic=5.0, adapt=4.8, engage=4.5, n=12）。Server PID 49987 on HEAD (20529ea)，含 AdaptabilityGate + voice shift + PvP distinctiveness + accuracy fix（arithmetic warning 含 computed value）。全部 live，restart 2026-04-10T18:15。
