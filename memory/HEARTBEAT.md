...
- [ ] 暖身賽2 等待啟動 — 規則寫「4 月初」，4/6 仍未啟動。**域名遷移**：`teaching-monster.com` → `teaching.monster`（舊域名 NXDOMAIN，新域名 200 OK）。API 狀態不變：`/competitions` → `[]`，`/api/competitions` → 404。SPA 需 browser 看完整內容。GitHub baseline repo 無新 commit。持續監控 <!-- domain-migration-discovered: 2026-04-06 -->
- [ ] End-to-end 測試（持續）
