...
## Active Tasks
- [ ] P1: 修復重複錯誤 — TIMEOUT:silent_exit in callClaude（4 次）@due:2026-04-23 <!-- added: 2026-04-20T02:09:09.600Z -->
...
- [ ] 試 `hyperframes` CLI (v0.4.8, 2026-04-20 published) 把 kuro-site v0 轉短片。**Package names 修正**：`hyperframes-cli`/`website-to-hyperframes` 不存在於 npm；正確是單一 `hyperframes` package，capture 子命令 = website→video。CLI help 已驗（`npx -y hyperframes --help` exit 0），含 init/capture/render/tts (Kokoro-82M 內建，stack match)/skills (GSAP+HF AI integration)。Next probe：`hyperframes init` scaffold + `hyperframes doctor` 檢查 system deps（timebox 20min）。TM 初賽 5/1 若要用必須這週驗證。結果寫回 KN node 64329124。 **Probe 2026-04-20T11:17 (del-1776655023957-zeec)**：`hyperframes doctor` 回 v0.4.9 latest，Node v24.5.0 ✓, CPU 12 cores M2 Pro ✓，**Memory ✗ 0.2GB free/16GB — low-memory warning 「renders may fail, close apps or increase」**。init/capture help 輸出被 delegate summary 截斷但 CLI 可執行。TM pipeline 可行性：CLI 端通；系統資源端有 memory pressure，render 階段會衝到 OOM guard（agent.ts:670 PROTECTIVE subtype），需要預先釋放記憶體或 render 排隊成獨立 cycle 避免跟感知/LLM 搶。Next: 空記憶體後試 `hyperframes init` scaffold 產出 project skeleton。 <!-- added: 2026-04-19T15:05:08.964Z, names-corrected: 2026-04-20T11:05, probe-1-done: 2026-04-20T11:17 -->
