...
- [ ] 暖身賽R2 等待啟動 — 規則確認：R2 題目由評審委員設計（非 LLM），更貼近初賽難度，定位「模擬考」。4/6 01:37 仍未啟動。平台 API 結構：comp 1(WR1原版,18隊) / comp 2(WR1重跑,12隊) / comp 3,4,5(elo制,空) — 3/4/5 是初賽 Arena 基礎建設。`/competitions` → `[]`。持續監控 <!-- domain-migration: 2026-04-06, api-deep-scan: 2026-04-06T01:37 -->
- [ ] End-to-end 測試（持續）
