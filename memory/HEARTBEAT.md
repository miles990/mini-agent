...
- [ ] **HN AI trend cron lag**（2026-04-30 update）：crontab `0 9 * * *` 存在，但實測連兩天延後 — 04-28 fetch 18:28、04-29 fetch 11:40 Taipei (`run_at` 03:40Z, 25 posts)。**問題不是 broken 而是 late**。可能根因：macOS cron 不會 catch up missed runs（pmset / caffeinate 對齊？）。enrich-remote 額外問題：Anthropic API 餘額不足 `ok=0 fail=20`（log: `[enrich-remote] credit balance is too low`）→ enrich 路徑斷，fetch 完仍 `summary.novelty="pending-llm-pass"`。下個 falsifier：04-30 09:30 看 `2026-04-30.json` 是否在 09:00-09:10 區間 `run_at`。
