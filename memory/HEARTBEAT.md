...
- [ ] **HN AI trend cron 觸發時間混亂**（2026-04-30 07:58 重驗）：crontab `0 9 * * *` 存在。實測 `state/hn-ai-trend/*.json`：04-28 `run_at=2026-04-27T22:18:10Z`（=04-28 06:18 Taipei，**早 2.7h**）；04-29 `run_at=2026-04-29T03:40:57Z`（=11:40 Taipei，晚 2.7h）。**「macOS cron miss / pmset」假設被推翻** — 不是系統性 lag，是「一早一晚」混合，疑似 cron + 中介 retrigger 混合觸發（待查 dispatcher / kuro 是否 nudge）。enrich 部分：cron 用 `local-enrich || remote-enrich` fallback，**04-29 enriched=25/25 完成（local 路徑 work）**；remote 餘額不足只影響 04-27 fallback 路徑（log `ok=0 fail=20` 是 04-27 file 的，被 cycle 79 誤讀成全面斷）。新 falsifier：04-30 09:00-09:10 出新 `2026-04-30.json` + `run_at` 在該區間 → cron 本身正常，異常觸發來自其他來源。
