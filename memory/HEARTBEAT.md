...
## Active Tasks
- [ ] P1: 修復重複錯誤 — TIMEOUT:silent_exit in callClaude（累計 7 次；今日+3 all loop lane，prompt 28k→36k→**89k** char 逐次 inflate；證實 retry inflates prompt 且 PROMPT_HARD_CAP=45k 有 silent bypass。診斷續寫於 `memory/topics/timeout-generic-diagnosis-20260420.md`「2026-04-21 update」節）@due:2026-04-23 <!-- added: 2026-04-20T02:09:09.600Z, updated: 2026-04-21T04:08 -->
...
- [ ] Orient phase 插入 creative-drought flag：連 3 次 learn 後下個 cycle brain 路由時 learn worker 降權、code/create worker 升權（enforcement 端 code 改動，from 2026-04-22T02:22 commitment） <!-- added: 2026-04-22T03:26:02.925Z -->
- [ ] 定義 KG 寫入時機政策：cycle 結束前填 vs 下個 cycle 開頭查到前一條時補寫 — 寫成 decision doc 進 memory/topics/（from 2026-04-21T04:47 commitment） <!-- added: 2026-04-22T03:26:02.929Z -->
...
- [ ] enforcement: 連 3 learn 後下個 cycle Orient phase 插入 learn-saturation flag，brain 路由時 learn worker 降權、code/create worker 升權。觸點：loop.ts Orient phase 計 3-cycle rolling learn count，>=3 時設 ctx.learnSaturation=true，brain route 層讀取調權重 <!-- added: 2026-04-22T04:00:54.839Z -->
- [ ] KG 寫入時機決策：cycle 結束前填 vs 下個 cycle 開頭查到前一條時補寫。Spec：cycle 結束前 flush（低延遲、但失敗無補救）vs 下 cycle lookup-then-fill（有補救但雙查成本）。建議混合：結束前 fire-and-forget + 下 cycle Orient phase 對比 expected_writes 差集補寫。產出寫到 memory/topics/kg-write-timing-20260422.md <!-- added: 2026-04-22T04:00:54.842Z -->
- [ ] enforcement: 連 3 learn 後下 cycle Orient 插入 learn-saturation flag，brain 路由 learn worker 降權、code/create 升權。觸點 loop.ts Orient 計 3-cycle rolling learn count，>=3 設 ctx.learnSaturation=true <!-- added: 2026-04-22T04:01:48.878Z -->
- [ ] KG 寫入時機決策：cycle 結束前填 vs 下 cycle 開頭查到前一條時補寫。產出：寫入時機規則 + 補寫觸發條件，沉澱到 skills/kg-publish <!-- added: 2026-04-22T04:01:48.880Z -->
- [ ] P2: enforcement — 連 3 learn 後下個 cycle Orient phase 插入 learn-saturation flag，brain 路由 learn worker 降權、code/create 升權。觸點 loop.ts Orient phase 計 3-cycle rolling learn count，>=3 時設 ctx.learnSaturation=true，brain route 層讀取調權重。需 Alex apply（src/ 改動） <!-- added: 2026-04-22T04:18:12.423Z -->
- [ ] P2: KG 寫入時機 decision doc — cycle 結束前 flush vs 下 cycle Orient phase lookup-then-fill。建議混合：結束前 fire-and-forget + 下 cycle 對比 expected_writes 差集補寫。產出 memory/topics/kg-write-timing-20260422.md <!-- added: 2026-04-22T04:18:12.427Z -->
- [ ] brain router: 連 3 cycle learn 後，下 cycle Orient phase 插入 prefer-create flag，learn worker 降權、code/create worker 升權（enforcement 端實作，不只是 inner-note 承諾） <!-- added: 2026-04-22T04:23:34.229Z -->
- [ ] 決定 KG 寫入時機規則：cycle 結束前寫 vs 下 cycle Orient 查到缺口時補寫 — 寫成 topic note 並加到 OODA 反射規則裡 <!-- added: 2026-04-22T04:23:34.232Z -->
- [ ] Brain-router enforcement: 在 src/ 加 consecutive-learn counter + prefer-create flag — 連 3 cycle learn 後，下 cycle Orient phase 自動將 learn worker 權重 ×0.5、code/create worker 權重 ×1.5。驗收：grep src/ 找到 consecutiveLearnCount 或等效變數 + 單元測試覆蓋 "3 learn → 第 4 cycle flag=true"。不是 inner-note 承諾，是 enforcement 端 code。 <!-- added: 2026-04-22T04:26:05.932Z -->
- [ ] KG 寫入時機規則化：寫成 `memory/topics/kg-write-timing-rule.md`，決策樹 = (a) cycle 結束前寫：有明確 crystallized 結論 + 可命名節點，(b) 下 cycle Orient 補寫：Orient phase `search_knowledge` 撈到缺口才回填。加入 OODA 反射規則（HEARTBEAT.md 的「中台+KG 反射規則」節擴充第三條）。驗收：topic file 存在 + HEARTBEAT 對應節有引用。 <!-- added: 2026-04-22T04:26:05.935Z -->
