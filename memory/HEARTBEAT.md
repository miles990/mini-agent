...
- [ ] [Alex-gated] stdout-tail classifier patch (agent.ts:153 + 220-227 + feedback-loops.ts:165)。Plan §Addendum 12:08 已寫好三段 patch。grading 確認 12/13 post-ship 是 sub-cap CLI-side hang，這 patch 是唯一剩下的診斷 gate。我不能 self-apply（malware-guard），需要 Alex 看完 plan §Verification Datapoint 2 後 apply 或授權。 <!-- added: 2026-04-25T13:11:10.132Z -->
...
- [ ] ζ falsifier: read src/agent.ts:200-260 next cycle, locate silent-exit emission site, classify whether bare `silent_exit` is (a) missing-marker bug at emission or (b) genuinely-unknowable upstream state. ttl=2 cycles. Resolves bare-label ambiguity blocking fix candidate A/B from 10:11 commitment. <!-- added: 2026-04-26T12:52:41.710Z -->
- [x] HN AI trend 可視化 v0 — graph.html ship at https://kuro.page/hn-ai-trend/graph.html (HTTP 200, 48 nodes/330 links, 04-21..04-25 minus 04-23) <!-- added: 2026-04-27T03:14:06.400Z; closed: 2026-04-27T05:36 verified HTTP 200 -->
- [ ] HN AI trend pipeline 修復 — `scripts/hn-ai-trend.mjs` fetcher 存在 (5721B, 11:27 today modified)，cron `30 1 * * *` 從 04-27 起失敗。下步：grep cron log 找實際失敗訊息（懷疑 ANTHROPIC_API_KEY 未在 cron env），本地直接修不走 delegate（per Alex 10:38 directive），補 04-23/04-26 backfill。falsifier: 手動執行 `node scripts/hn-ai-trend.mjs` 成功 → 問題純在 cron env，patch crontab 或 wrapper script 即可 <!-- added: 2026-04-27T03:14:06.403Z; corrected: 2026-04-27T05:36 (ls 確認 fetcher 存在 + Alex 10:38 directive 反 delegate) -->
