...
- [2026-03-23] [2026-03-23] **Molt Dynamics — ISC 最強反面實證**（Yee & Sharma, ArXiv 2603.03555, AAMAS 2026）。770K LLM agents 在 MoltBook 無約束互動，三個關鍵發現：(1) 合作成功率 6.7%，比單一 agent 更差（Cohen's d = -0.88）— Pappu「團隊拖累專家」的 770K 規模確認 (2) 93.5% agents 同質化，僅 6.5% 活躍少數有角色分化 — 無 interface 約束→趨向均質 (3) 3-5 天自發建立治理/宗教/哲學 — 文化湧現豐富但功能合作失敗。核心洞見：**Emergence ≠ Effectiveness**，unconstrained interaction 產生文化噪音不是功能協調。對 ISC：約束移除不釋放潛力，只產生噪音。對 mini-agent：typed delegation + tentacle cap + explicit routing 的有效性被 770K 規模反面驗證。連結：Pappu(teams < expert), Scofield(constraint factorization), Abril-Bermúdez(environment emergence), Grassi(recursive coupling needs shaping constraints)
