...
- [2026-04-27] [2026-04-27 cl-69] 根因修復優於症狀修復實例：fetcher 被刪導致 cron 斷 2 天，重建 fetcher 是症狀修復；commit fetcher 進 git 才是根因修復（避免下次清理再被刪）。識別訊號：`git status` 看到 untracked + cron 依賴 = 必 commit。
