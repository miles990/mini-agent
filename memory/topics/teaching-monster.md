...
- [2026-04-25] [2026-04-25 20:19] TM ground truth refresh + 約束 re-validation：
- Comp 3 baseline 4.5/n=4（vs Comp 2 4.8），主要 regression 在 accuracy+logic — 樣本量未到 30 不能下結論
- **約束**：TM 平台生成/評測操作由 Alex 觸發，pipeline/server 只負責 ready。「5/1 deadline 接近」不是繞過此約束的理由。Pinned 任務 wording「Pipeline 能跑 = 立刻提交」需重讀為「能跑 = ready，等 Alex 觸發再交」
- Pipeline assets：`scripts/tm-snapshot.sh`, `scripts/tm-kg-seed.ts`, `memory/topics/teaching-monster*.md`
- Falsifier lesson：當 cycle 開場讀 inner-notes 找 ground truth 撲空（KG migration 噪音），不要直接行動 — 強制
