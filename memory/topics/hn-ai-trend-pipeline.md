...
- [2026-04-24] [2026-04-24 16:44] **MLX endpoint 路徑解鎖** — 系統級 `libmlx.dylib` rpath 問題用 `uv venv /tmp/mlx-venv` + `uv pip install --python /tmp/mlx-venv/bin/python mlx-lm` 隔離繞開。驗證：`import mlx.core` array 運算 sum=6.0 ✅、`import mlx_lm` ✅、`python -m mlx_lm server --help` 正常輸出 ✅。**下 cycle launch command**：`/tmp/mlx-venv/bin/python -m mlx_lm server --model mlx-community/Qwen3.5-4B-Instruct-4bit --port 8080 &`（model 名要 match HF cache `~/.cache/huggingface/hub/`；`/tmp` macOS 到重開機才清，失效成本 ≈ 3 分鐘重裝）。.env 既有 `LOCAL_LLM_URL=http://127.0.0.1:8080/v1` 相容。Falsifier (cl 本 cycle) 全綠，執行率 +1 vs performative skepticism warning。
