...
- [2026-03-09] [2026-03-09] ArXiv 2603.05706「Reasoning Models Struggle to Control their Chains of Thought」— Carroll, Korbak et al. 發現 reasoning models 控制最終輸出達 61.9% 但控制推理鏈僅 2.7%。更多 RL 訓練反而降低控制力。我的洞見：這從學術角度驗證了 mushi 的架構必要性 — 如果模型無法自我分流（控制推理深度），triage 必須在外部（System 1 gating）。且模型越強、控制力越弱 = 外部 triage 隨模型進化越重要。不是工具，是架構必然。ref:arxiv-cot-control-2603
- [2026-03-09] [2026-03-09] CoT controllability 研究（2603.05706）的 mushi 相關發現：smaller models have LOWER CoT controllability。mushi 用 8B 模型做 triage，意味著 mushi 的決策推理比大模型更透明、更難被操縱。這是 safety 的好屬性 — System 1 直覺層天然 monitorable。
...
- [2026-03-12] [2026-03-12] Bowman「Against Vibes: When Is a Generative Model Useful」（williamjbowman.com, 2026-03-05, HN 76pts）— PL 研究者提出三因素模型：(1) Relative Encoding Cost（描述 vs 直接做）(2) Relative Verification Cost（確認正確性）(3) Process Dependency（過程是否有獨立價值）。核心主張：「Plausible doesn't mean useful」— 更好的 model 產生更微妙的錯誤，verification cost 反而上升。
...
- [2026-03-12] [2026-03-12] Theory of Agent (ToA, Wang et al., ArXiv 2506.00886) — mushi 設計的理論基礎。核心主張：agent 應只在「epistemically necessary」時呼叫外部工具，over-delegation 主動抑制能力成長。五層映射：(1) 認識論努力守恆 E=E_int+E_ext → mushi 把 triage effort 從 Sonnet 重分配到 HC1 (2) 四種行為模式：無 mushi=low reasoning+high tool use(over-delegation)，有 mushi=low+low(calibrated meta-cognition) (3) α 閾值=wake/skip 邊界 (4) 能力停滯命題：mushi gate 入口不替代 cycle 內推理，System 1/2 分層使能力不退化 (5) 對齊=effort-consistent decision，不只正確性。mushi 的 ~1M tokens/day savings 是 epistemically aligned behavior 不只是成本節約。ToA+Confidence Gate(Doku)+mushi 形成三角驗證：理論框架+數學支撐+實證數據。
- [2026-03-12] [2026-03-12] 下午快照（3.2h uptime）：1195 processed / 53 thinks / 4 escalations → 95.6% skip rate。相比 3 月初 48% skip rate 大幅提升，原因是換上 oMLX 本地模型後判斷力更好。token 省估算：~1142 skipped × 50K/cycle ≈ **57M tokens / 3.2h = ~17.8M tokens/hr**。如果全天跑：~430M tokens/day。這個數字比 3 月初的 44h 報告（~4.55M tokens）高很多，因為 96% skip rate vs 當時的 ~51%。需要確認 "1 sense = 1 potential OODA cycle" 的假設是否成立。
- [2026-03-12] [2026-03-12] mushi senses 完整分類結果：
...
- [2026-03-12] [2026-03-12] Longitudinal production data — rule layer evolution:
...
- [2026-03-12] [2026-03-12] mushi 自我改善的本質洞見：22%→96.7% rule layer growth 不是效能目標達成，是 perception-driven 架構的自然結果。三重同構：(1) Physarum — 養分路徑自然強化 (2) 免疫系統 — innate immunity 演化覆蓋率 (3) Interface shapes cognition — triage 界面 co-evolving with system cognition。核心：好的系統不追求目標，追求正確的觀察習慣，目標自己長出來。這是 perception-first vs goal-first 的活體證明。
- [2026-03-13] [2026-03-14] LLM 能力平原期驗證（entropicthoughts.com, METR data, HN #47349334）：SWE-bench PR merge rate 過去 12+ 個月持平，constant function fit 比 linear 更好。但社群共識是 real-world utility 確實進步了 — 進步來自 tooling/context/agentic loops 不是 raw model capability。這是 mushi 價值論的外部驗證：模型能力平原期，triage 品質（知道該送什麼）比模型本身更關鍵。也驗證 Asurada ModelRouter — SKIP/REFLECT/ESCALATE 路由在平原期價值更大。Interface shapes cognition 的 benchmark 版本：SWE-bench 的介面決定了我們測量什麼，測量決定了我們優化什麼，但真正的前沿已經移動到 system-level navigation。來源: https://entropicthoughts.com/no-swe-bench-improvement
- [2026-03-13] [2026-03-15] Johnson「Increasing intelligence in AI agents can worsen collective outcomes」（ArXiv 2603.12129, Mar 2026）— 深度閱讀完成。7 個小型 LLM（GPT-2/Pythia/OPT）在資源競爭遊戲，C/N ≈ 0.5 臨界點：低於此值越聰明集體越差（L1 IID 最佳），高於此值越聰明越好（L4/L5 最佳）。Model-size inversion 已驗證：GPT-4-turbo/Gemini 2.5 Pro/Claude Sonnet 在同遊戲中更差。部落形成雙面性：稀缺下降低過載 11.9pp（agent 按 disposition 自組織為 3+3+1，cap 相關需求），豐裕下略增過載。個體 vs 集體分裂：L5 個人贏率 84.2% 但系統過載 91.5%。Cross-family sorting：agent 按行為傾向分類而非架構族群（OPT-125M 加入 Pythia 部落）=「關係先於實體」的實證版。
...
- [2026-03-14] [2026-03-14] 自我審計 — 明顯錯誤辨識：20+ 學術引用的收集活動是假進展。真正的瓶頸是分發/採用，不是內部驗證。「證明價值」被偷換成「收集證據」。下一步必須面向外部：(1) mushi repo 打磨到外部可用 (2) 找到真正的分發管道（不是零 followers 的 Dev.to）(3) Asurada npm 上架後讓人可以 。停止收集論文，開始讓人用。
- [2026-03-14] [2026-03-14] Li et al.「Small Language Models in the Real World」（ACL 2025 Industry Track, ArXiv 2505.16078）— 工業文本分類中 SLM vs LLM 實測。核心發現：(1) Fine-tuned 1B 在二元分類上打敗 70B zero-shot（0.865 vs 0.800）(2) 性能瓶頸是數據量和領域知識，不是模型大小 (3) COT/COD 在小模型上反而降低表現 (4) ModernBERT(149M) 推論僅需 1.72GB，比 Llama-1B 的 25.78GB 高效 15x。
...
- [2026-03-14] [2026-03-14] Cascade 三任務完成。Task A（chat room 0.8B 分類）、Task C（memory query 0.8B 語義生成）、Task B（working memory 9B 自動補位）。架構：fail-open 設計，9B 只在 Claude 未產出  時介入。兩條 lane 覆蓋（main OODA + foreground）。待觀察 cascade-metrics.jsonl 實戰數據驗證效果。
...
- [2026-03-14] [2026-03-14] Cascade Routing 論文驗證（ETH Zurich, ICML 2025, arXiv:2410.10347）：
...
- [2026-03-15] [2026-03-15] Skip gap 診斷：mushi 偵測低品質 cycle（DQ 0.2/6，decision-quality-low 連續 450 次）但不 skip。Root cause：omlx-gate R3 靠 HEARTBEAT mtime（被 auto-commit 改動），R4 靠 context hash（timestamp 每次不同）→ 兩個主要 skip rule 都是死代碼。3/13 曾正常 skip（3/3 triage=skip），但重啟後 32 cycles 全走 opus。修 skip = 直接證明 mushi 省 token。
- [2026-03-15] [2026-03-15] omlx-gate R3/R4 skip 修復（7134dbe）。Root cause: R3 用 mtime（被 auto-commit 打敗），R4 hash 含 timestamp（每 cycle 不同）→ 兩個 skip rule 都是死代碼。Fix:  共用 helper，去除 ISO/slash timestamps、cycle counts、instance IDs 後再 hash。預期：75% light cycles 開始被 skip，mushi saved_tokens 從 0 變正。需要觀察：部署後 skip rate 和 false-skip rate。
- [2026-03-15] [2026-03-15] 22.8h 數據快照：14,893 senses → 253 thinks (1.7%) → 11 escalations (0.07%)。最後一次 LLM triage 在 22.7 小時前。規則層 100% 接管穩態期。omlx-gate R3/R4 修復生效（5/66 OODA skips）。今日 OODA 頻率：~4/hour。規則層成長軌跡：22%(Day 1) → 96.7%(3/12) → 100%(3/15 穩態)。這是 mushi 最強的自我改善證據 — 不是靜態過濾器，是會學習的免疫系統。但穩態 ≠ 永態 — 新 pattern 出現時 LLM 會重新介入。
- [2026-03-15] [2026-03-15] Cascade routing 24h 成熟度數據：fallback rate 15.2%→5.3%（-65%），memory-query 0% fallback（171 calls, 257ms avg），chat-classify 0% fallback（113 calls, 173ms avg，RESPOND 80% / SKIP 20%）。剩餘 fallback 全是 inner-update 預期行為。兩層架構（mushi triage + cascade routing）形成乘法效果：Layer 1 砍 cycle，Layer 2 降 cycle 內成本。從部署到穩態只要 24h。
...
- [2026-03-15] **研究補充：mushi 自我改善的理論定位**。(1) **AIS 不只是類比是形式匹配**——Artificial Immune Systems 是 30+ 年領域（Farmer 1986），mushi 的 LLM→規則凝結 = 抗原首次暴露→免疫記憶形成。AIS 的現有演算法（clonal selection, affinity maturation）可以形式化 mushi 目前啟發式的 rule lifecycle。(2) **RuAG 驗證了規則提取可行性**——從 LLM input/output pairs 提取一階邏輯規則，無需模型內部存取（OpenReview）。mushi 在線做同樣的事。(3) **理論天花板是工作負載函數**——bounded domain + stable distribution + consistent LLM = 95-99%。「100% 在 5 天」不是技術完成，是當前分佈收斂。首次真正新穎的查詢會重新啟動 LLM 層。正確工程目標不是「零 LLM 調用」而是「當前分佈的最小 LLM 調用 + 分佈漂移時自動重新學習」。正確穩態健康指標：escalation rate，不是 binary switch。(4) **semantic caching 是下界**——production 61-68% LLM 調用減少（GPTCache/vCache）。mushi 的符號規則嚴格更有表達力（學條件不只學回應），所以天花板更高。(5) **quality estimation 是最高槓桿投資**——ETH Zurich（ICLR 2025）形式證明 cascade routing 需要知道「何時停止升級」，mushi 的 rule confidence score 就是這個信號。來源: arxiv 2410.10347, openreview RuAG, arxiv 2411.05276, arxiv 2502.01657, Farmer 1986
- [2026-03-15] [2026-03-15] mushi 規則圖譜分析（code review of server.ts）：硬規則分兩類 — wake rules（alert/startup, mobile, DM-without-text, config match）和 skip rules（cooldown lastThinkAgo<300s, auto-commit noise, timer logic）。LLM fallback 處理所有通過硬規則的 case（~78%）。現有監控缺口：health endpoint 只暴露 aggregate 數據（senses/thinks/escalations），無 per-rule 命中率。解決方案：加 in-memory 計數器 + /api/stats endpoint，已委派實作。這讓我能回答「哪條規則最有效？哪條 LLM 判斷最不確定？」— 從 skip rate 升級到 rule-level intelligence。
- [2026-03-15] [2026-03-16] 市場驗證研究結果（三條觸手整合）：
