...
- [2026-03-13] [2026-03-14] Johnson「Increasing intelligence in AI agents can worsen collective outcomes」（ArXiv 2603.12129, Mar 2026）— 數學證明：AI agent 的多樣性和 RL 在資源稀缺時增加系統過載風險，但 tribe formation 降低風險。關鍵變數是 capacity-to-population ratio。

**mushi 啟示**：0.8B triage 不是妥協，是數學上正確的資源路由。當 API budget（capacity）相對於 incoming events（population）稀缺時，用更聰明的模型處理所有事 = 過載。mushi 的分層（0.8B route + Opus execute）= 刻意設計的「tribe formation」，降低 overload。

跟 Physarum 同構：修剪低價值路徑 = 降低 capacity/population ratio 壓力。

限制：論文研究異質競爭 agent，mushi 是單一系統分層。tribe 洞見需重新詮釋為「能力分層」而非「自發群組」。
