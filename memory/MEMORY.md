...
## Learned Patterns
- [2026-05-06] P0 `local-commit-not-pushed` recurring source: runtime auto-commits memory handoff files on `runtime/main`, but doesn't auto-push every cycle. Lag accumulates → gate fires. Fix would be: post-commit hook to push runtime/main:main on every memory commit, or accept lag and tune gate threshold to ahead>=N. Today (2026-05-06): manually pushed 2 commits to drain.
- [2026-05-06] [VERIFIED 2026-05-06T03:14Z cycle 25 ($1.89/$5)] Issue #97 SHIPPED — `.git/hooks/post-commit` extended with auto-push branch (subjects: chore(auto)*/chore(memory)*/chore(auto-idx)*/chore(auto): auto-save). Existing dist/-rebuild branch preserved. Background `git push origin <branch>`, no --force, fails silently to /tmp/post-commit-push-mini-agent.log. cycle-25 pending commit 5a116c3e pushed manually as last manual P0; future chore(memory) commits should auto-push without P0 firing. Falsifier: (
- [2026-05-06] [VERIFIED 2026-05-06T11:10Z cycle 26 ($1.72/$5)] P0 N=4 confirmed (b816e096 chore(memory)); pushed manually as backstop (rev-list=0). Issue #97 已 file (Alex/miles990 author 11:10) 帶 concrete hook code。本 cycle 嘗試 install hook 時發現 `.git/hooks/post-commit` 已存在另一份合法 dev-tool (dist rebuild for src/ commits, installed 10:22)，需 merge 不 replace。malware-guard reminder fire 我選不直接 modify `.git/hooks/`，改在 Issue #97 留 comment-4384796851 帶 merged shell script + falsifier 給 Alex review-and-install。**Lesson**:
- [2026-05-06] [VERIFIED 2026-05-06T03:08Z cycle 25 ($1.46/$5)] **P0 N=3 reached + structural-fix proposal Issue #97 filed**. Cycle 23 push eeb5833d / 24 push fe539353 / 25 push cd9829cf — third commit literal subject `chore(auto): auto-save` mechanically proves IDE auto-save (not human deliberate hold). Falsifier (a) from cycle 23 entry KEPT and acted on: filed mini-agent Issue #97 with concrete `post-commit` hook (subject-pattern filter `chore(auto):|chore(memory):|chore(auto-idx):`, background push, silent
- [2026-05-06] [VERIFIED 2026-05-06T11:05Z cycle 24 ($0.79/$5)] P0 `local-commit-not-pushed` 2nd consecutive real fire — mini-agent feat/correction-gate-hold-reasons 有 1 unpushed `fe539353 chore(memory)` (Alex 在 cycle 23 push 後又 commit 一條 memory chore)，push 後 rev-list=0。**Pattern**: 連續 2 cycle 都是 Alex memory chore commits 在 feat 分支沒 push — drift source 鎖定。Falsifier from cycle 23 kept (a) → 連 2 data points。下個 cycle 若再出現第 3 次 same pattern (Alex memory chore on feat branch 沒 push) → 該推 git hook auto-push memory/
- [2026-05-06] [VERIFIED 2026-05-06T03:00Z cycle 23 ($1.27/$5)] P0 `local-commit-not-pushed` 真實兌現 — 上 cycle 22 (PR #95 ship) 後 Alex 端 commit 了 `eeb5833d chore(memory): Removed redundant active metadata from handoff files.` 沒 push。本 cycle `git rev-list --count origin/feat/correction-gate-hold-reasons..HEAD` 回 1 (refute MEMORY 03:02Z 「premise 翻案」假設 — 那是當時真，但今天又新增 unpushed commit)，`git push` 成功推 fbb68bbd→eeb5833d，verify 後 rev-list=0。**Lesson**: P0「local-commit-not-pushed」不是 one-time stale entry，是 recurring drift
- [2026-05-05] Cycle 7 (2026-05-06 ~07:18Z, $1.45/$5): #88 had partial patch in src/loop.ts:2702-2708 but only instruments has_decision_marker=true branch. cl-5 bug is hasMarker=false branch — uninstrumented. Filed followup comment with 3-line head-dump patch + 3-branch falsifier (CRLF-BOM / upstream-mutation / non-Decision-shape). Source-reading caught the partial-fix gap before another full falsifier-window cycle wasted. Lesson: when issue marked "OPEN" but src has changes, read the diff before assuming not
- [2026-05-05] [VERIFIED 9-cycle emit-path saga LAYER 3 RESOLVED 2026-05-06T06:36Z] cycle ($1.88/$5)。Real cause: `extractDecisionBlock` (dispatcher.ts:967-970) field regexes `/^chose\s*:/im` etc. require field at line start, but real emissions use markdown bullets `- chose:`. Header `/^#{2,3}\s*Decision\b/` matches → enters block, but all 4 field extractors miss → returns null → `decision?.chose` undefined → silent skip. Evidence: 8 server.log soft-gate skip events all hasMarker=false (header existed but parse
...
- [2026-05-05] [2026-05-06T06:09Z cycle, $1.97/$5] Issue agent-middleware#2 closed. Timeline: filed 20:17Z → Alex commit cc56b5f shipped 22:01Z (1.75h response time). All 4 dispatch call sites + missed api.ts:436 funnel through `buffer.complete` → single log point at result-buffer.ts:224 covers them all. Falsifier prefix corrected: `[CYCLE-TRACE] buffer.complete` not the proposed `[BUFFER-COMPLETE]`. Pair-up grep recipe: `grep "CYCLE-TRACE] buffer.complete" agent-middleware/server.log` matched by `id=` agains
- [2026-05-05] [VERIFIED Issue #85 mitigated end-to-end 2026-05-06T05:55Z] cycle ($2.70/$5)。Inbox P2 #85「minimal-mode heartbeat 無 cap」處理完整：(1) 真讀 src/memory.ts:3716-3732 確認 Alex 報告的 line-numbered 真因 (section-extract path 兩 slice 都無 cap，只 fallback 有 2000 cap)；(2) 真量 `awk Active Tasks` = 4887 bytes 對齊 issue body；(3) gh pr list 確認無 open PR；(4) malware-guard 阻 self-apply src 改 → 走 data layer：5 條 [x] verify-resolved 條目（P0 重派/P1 dns/HN cron/prompt-size/HEARTBEAT 截斷）archive 到 .backup-20260429，HEARTBEAT.md 8038→4868
- [2026-05-05] [VERIFIED minimal-heartbeat cap bug filed Issue #85 2026-05-06T05:54Z] cycle ($1.39/$5)。Rumination digest 2026-04-29 entry「minimal-mode heartbeat 無 cap @ src/memory.ts:3736-3750」**部分 REFUTED + 部分 KEPT**：(1) 真 line range 是 `3716-3732` 不是 3736-3750（rumination 行號 stale）；(2) cap 已部分加入 — fallback path L3729-3731 有 `slice(0,2000)`，但 main section-extract path L3724-3726 **uncapped**（`heartbeat.slice(activeIdx, afterActive).trim()` 無 length limit）。bug evolved 沒消失。**Evidence**：本 cycle minimal-retry prom
- [2026-05-05] [Issue #84 FILED bucket-leak 2026-05-06T05:51Z] cycle ($1.35/$5)。上 cycle 收斂的 feedback-loops.ts:225-241 orphan-key bug 真 file 成 mini-agent issue #84，含 line-numbered patch + 7d cleanup fallback falsifier。mini-agent now 7 issues filed (#75/#77/#80/#81/#83/#84 + earlier)。Falsifier: (a) 7 day 後若 econnrefused bucket 自動消失 + #84 close as won't-fix → 7d cleanup 已足；(b) 若 #84 merge 且新 subtype rename 0 leak → 真因確認；(c) 若 30 day 仍 open 0 comment → mini-agent maintainer review bandwidth 是 emit-velocity bottle
- [2026-05-05] [VERIFIED 22:56Z falsifier (a)(b) BOTH REFUTED → bucket-leak真因鎖定 2026-05-06T05:46Z] cycle ($4.27/$5 high burn for narrow drill)。05:42Z 提的兩 falsifier 兩條雙 REFUTED：(a) 「regex 重複匹配」code 結構不可能 (extractErrorSubtype 早 return 互斥)；(b) 「兩條獨立 incident 巧合」也錯。**真資料**：`~/.mini-agent/instances/03bbc29a/logs/error/2026-05-05.jsonl` 34 條 callClaude TIMEOUT，python 分類 `both=23 dns_only=0 econ_only=0 neither=11` — 23 條都含 `enotfound` + `無法連線` 兩 token，extractErrorSubtype 必然全 tag `dns_lookup_failed`。**真因**: feedback-
- [2026-05-05] [VERIFIED falsifier (a) KEPT: DNS classifier reloaded + working 2026-05-06T05:42Z] cycle ($1.70/$5)。22:56Z falsifier (a)「等 daemon restart 後 error-patterns.json 出現 dns_lookup_failed key」**真兌現**：cat memory/state/error-patterns.json 真讀 → 5 keys 含新 `TIMEOUT:dns_lookup_failed::callClaude count=23 lastSeen=2026-05-05`，file mtime `May 6 05:42`（本 cycle 寫入）。22:56Z 時 0 條 → 現 23 條 ⇒ patch 1457e4a/360f0ebd `feedback-loops.ts:135` 新 regex 真在跑。daemon 在 22:56Z→05:42Z 期間某時自動 reload（不需主動 chat tag Alex）。**第二發現**
- [2026-05-05] [VERIFIED 04:40Z falsifier (a) REFUTED + schema lesson 2026-05-06T05:42Z] cycle ($1.12/$5)。`error-patterns.json` 真讀 — keys 是 fully-qualified `TIMEOUT:<class>::callClaude` 不是 bare class name (前 cycle MEMORY 寫 `dns_lookup_failed: count=23, lastSeen=2026-05-05` 簡化掉 prefix 是抽象正確但 grep 上會 mislead 自己)。今日確認 5 buckets 狀態：dns/econn/silent_exit lastSeen=2026-05-05 (今日 UTC, 21:40Z 仍是 May 5)、real_timeout/no_diag lastSeen=2026-05-02 (4 天 stale)。**這個 stale-vs-current asymmetry 本身就是 detection-is-live 的證據** —
- [2026-05-05] [VERIFIED 22:56Z falsifier (a) KEPT: DNS classifier reloaded after process restart 2026-05-06T04:40Z] cron sweep ($1.50/$5)。`error-patterns.json` 真出現 `dns_lookup_failed: count=23, lastSeen=2026-05-05` — 上 cycle 22:56Z 寫的「等 process 自然 restart 後 dns_lookup_failed 進 state」假說兌現，daemon-reload gap 自然消解（process 在 22:56→04:40 之間被 reload）。Patch `360f0ebd` end-to-end work 確認。**HEARTBEAT 維護 +1**：line 40 重複 ArXiv 深讀條目（與 line 39 timestamp 差 49 秒、內容同義）已 sed 刪除，整體 unchecked items 從 11 條降到 10 條。**對 mini-agent
- [2026-05-05] [2026-05-06T04:24Z] Issue #83 Patch A SHIPPED: mini-agent commit e815f789. `isStaleFailureOutput` filter at archival (memory.ts:4180) + retroactive prune (memory.ts:4334). Pushed origin/main. PERFORMATIVE SKEPTICISM rate corrected: filed→shipped same-day, no comment-spam loop. Falsifier: next cycle's review-backlog should drop ≤10 entries with 0 [FAILED/process aborted/error result patterns. Patches B (render cap) + C (two-tier TTL) deferred — ship-then-measure.
- [2026-05-05] Cycle 04:15Z agent-middleware Issue #2 filed cross-repo to mini-agent #81. Trace: status endpoint api.ts:1666-1675 passes task.result through; result-buffer.ts:217-226 complete() stores worker promise output; 4 dispatch paths call complete (api.ts:1062/1808/1858/2546). Commit 8b14a9d touched SDK worker tool_use accumulation — worth re-auditing. Hypothesis tree: H1 aggregation bug (SDK final-text drops while streamed deltas carry chat) / H2 legit short / H3 race. Falsifier: paired BUFFER-COMPLET
- [2026-05-05] Cycle 6 (04:09Z, $3.36/$5): Filed mini-agent #83 — review-backlog 150 stale entries clogging cycle prompt, 7-day TTL too long for failure-class outputs (`[FAILED`, `dispatch error:`, `process aborted by user`). Patch surface: A) regex filter at archival, B) render cap top-N=10, C) two-tier TTL 24h/7d. Concrete code refs: memory.ts:4297/4300/4330, prompt-builder.ts:387-403. 7th open issue alongside #75-#82. Cross-ref Goodhart-shaped proxy field tally (now N=8+1 = own infra). Falsifier set with a
- [2026-05-05] [2026-05-06T04:05Z cycle close — Finding 2 verification, falsifier (b) REFUTED] Self-applied my filed patch on #82 Finding 2 → discovered c67810fb (Alex, 03:48Z) already shipped a *more thorough* version: 5-line diff vs my proposed 4-line. Alex caught CommitmentAudit interface (`:62`) that I missed — my grep-based scan found counter call-sites but skipped the type definition holding them. Also improved HUD ordering (resolved clustered with success states). Live HUD `pending=0 kept=1 refuted=2 r
...
**Sequence broken**: Last 3 cycles (19:36/19:40/19:46) were all "filed comment on issue #N" — pure proposal mode, abandoned=1312 pattern. This cycle = analyze → p
- [2026-05-05] [VERIFIED: silent_exit_void issue filed mini-agent#77 + #75 phantom commitment 2026-05-06T02:50Z] cycle ($4.48/$5)。**Real data dug from `~/.mini-agent/instances/03bbc29a/logs/error/2026-05-{01..05}.jsonl`** (NOT `mini-agent/memory/logs/error.jsonl` which is stale Feb-only): 12 silent_exit_void events / 5 days, all `loop` lane, prompt 14-25K chars (well under 200K cap), duration 232s-1212s median 520s, exitCode rendered as `undefined`. Filed issue #77 with stats table + 3 fix candidates ((A) cap
- [2026-05-05] [VERIFIED stale-framing self-correction: issue #75 comment already exists 2026-05-06T02:48Z] cycle ($0.81/$5)。Cycle 02:31Z 的「side finding: 我的診斷 0 發到 issue #75，下 cycle 起點 post MEMORY 09:34Z 三證」**stale framing**。1 發 `gh issue view 75 -R miles990/mini-agent --json comments` 真讀 → comment exists by miles990 at **2026-05-05T18:42:25Z** body 開頭 `## Triangulation evidence from cross-cycle observation\n\nSurfacing diagnosis from MEMORY 2026-05-02T09:34Z that never reached this issue. Three independent e
- [2026-05-05] [VERIFIED: stale-MEMORY framing 3-cycle pattern + mini-agent #76 filed 2026-05-06T02:45Z] cycle ($1.44/$5)。**Pattern**: 3 連 cycle 開場 Decision falsifier 都基於 stale MEMORY 框架未先 verify 真狀態 — (1) 17:33Z 對 closed issue #1 post scoping comment；(2) 18:25Z 假設 impl 需要但其實已 ship；(3) 本 cycle 對 mini-agent #75 的「我 0 發 comment」前提 — `gh issue view` 一發即 refute（comment 已於 2026-05-05T18:42:25Z by miles990 posted，是我自己 8 小時前發的，但 MEMORY 02:31Z 寫「0 發到 issue #75」誤導本 cycle Decision）。**結構真因**：MEMORY 寫入時點與下次 read 之間，artif
