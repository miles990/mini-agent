# Mini-Agent — Operating Preferences (Reference)

> Moved from `CLAUDE.md` on 2026-04-25 during the de-accretion prune (KG `1c2885cd`, proposal `2026-04-25-kuro-deaccretion-dag.md`).
> Auto-loading was removed; this is a reference doc — read it on demand. The lean CLAUDE.md retains only hard rules. Architecture, system descriptions, agent tags, debugging tips, and historical evolution live here.
...
**Docs**: `docs/diagrams/mini-agent-architecture.png`（主架構圖，gpt-image-2 生成）
...
**可用工具**：curl、`cdp-fetch.mjs`（Chrome CDP：fetch/screenshot/inspect/click/type/interact/watch/network/login，port 9222）、Grok API（X/Twitter）、docker CLI、`gh` CLI、Claude CLI subprocess、`codex` CLI（OpenAI，第二意見 + image gen）、FTS5 搜尋
...
**Codex CLI**（`skill-codex:codex` skill）：OpenAI 的 coding agent CLI，用於 second opinion、跨 AI 辯論、圖片生成。本機 `~/.codex/config.toml` 預設 `model_provider = "omlx"`（本地 MLX），**要走 OpenAI 雲端必須 override**：`codex exec -c model_provider=openai -m gpt-5.5 ...`。圖片生成：prompt 內加 `$imagegen`（走 `gpt-image-2`，輸出到 `~/.codex/generated_images/`，需要自己 `cp` 到目標路徑）。gpt-5.5 僅支援 ChatGPT 登入，不支援 API key。
